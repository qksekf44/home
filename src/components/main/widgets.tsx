...
function useEditEvent(id: string, onOpen: () => void) {
  useEffect(() => {
    const h = (e: Event) => {
      if ((e as CustomEvent).detail?.id === id) onOpen();
    };
...
export function BannerWidget({ conf }: { conf: WidgetConf }) {
  ...
  const slides =
    ((conf.settings.slides as BannerSlide[]) ?? []).length > 0
      ? (conf.settings.slides as BannerSlide[])
      : DEMO_SLIDES;
  ...
  return (
    <div
      className="banner"
      style={{ cursor: s.link && !editOn ? "pointer" : undefined }}
      onClick={go}
    >
      {slides.map((sl, i) => (
        <div
        ...
        >
          {sl.img ? (
            ...
            >
              <span>SLIDE BANNER {String(i + 1).padStart(2, "0")}</span>
            </div>
          )}
        </div>
      ))}
      ...
      <div className="dots" onClick={(e) => e.stopPropagation()}>
        {slides.map((sl, i) => (
          <i
            key={sl.id}
            className={i === Math.min(cur, slides.length - 1) ? "on" : ""}
            onClick={() => setCur(i)}
          />
        ))}
      </div>
    ...
    <div className="panel menu-list wgt-menu">
      {(menuLoaded && boardsLoaded
        ? buildMenu(
            ...
            [
              ...boardEntries(boards),
              ...sectionMenuEntries(wSecMap),
              ...linkEntries(wLinks),
            ],
            { loggedIn: !!wUser, isAdmin: wIsAdmin, id: wUser?.id },
          )
        : []
      ).map((m) =>
        m.children ? (
          ...
          >
            <a onClick={() => setOpen((o) => (o === m.label ? null : m.label))}>
              {m.label}
            </a>
            <div className="msub">
...
export function DiaryWidget() {
  ...
  const latest = posts
    .filter((p) =>
      canViewHref(menuSet, sectionHref("diary", p.secId ?? MAIN_SEC), viewer),
    )
    .filter(
      (p) => p.visibility === "public" || (p.visibility === "member" && !!user),
    )
    ...
    <div className="panel widget" style={{ margin: 0 }}>
      ...
      {latest.map((p) => {
        ...
        return (
          <div
            key={p.id}
            className="diary-mini"
            onClick={() => router.push(`/diary#${p.id}`)}
          >
            <div
              className="mood"
              style={{
                background: moodTint(m?.color ?? "#888"),
                color: m?.color,
              }}
            >
              {m?.icon ?? "·"}
...
export function LatestWidget() {
  ...
  const latest = [
    ...(seeRoad ? roads : [])
      .filter((it) =>
        canViewHref(
          menuSet,
          sectionHref("roadview", it.secId ?? MAIN_SEC),
          viewer,
        ),
      )
      .map((it) => ({
        id: `r-${it.id}`,
        date: it.date,
        ref: it.imgId ?? it.imgUrl,
        ph: it.ph,
        href: "/loadb",
        tip: `로드비 · No.${String(it.no ?? 0).padStart(3, "0")}`,
      })),
    // 갤러리 — 전체공개 + 접기/비밀번호/스포일러/성인 제한이 없는 게시물의 대표(첫) 이미지
    ...(seeGal ? backups : [])
      .filter((p) =>
        canViewHref(
          menuSet,
          sectionHref("gallery", p.secId ?? MAIN_SEC),
          viewer,
        ),
      )
      .filter((p) => {
        ...
        // 2. fold.type이 spoiler나 adult인 경우 제외
        const foldType = (p.fold as { type?: string } | null)?.type;
        if (foldType === "spoiler" || foldType === "adult") return false;

        // 3. 비밀번호가 설정된 게시물 제외
        const hasPassword = Boolean(
          p.password ||
          (typeof p.type === "object" &&
            (p.type as { password?: string })?.password),
        );
        if (hasPassword) return false;

        // 4. 그 외 fold가 단순 접기로 존재하는 경우에도 숨기려면 유지 (필요에 따라 조절 가능)
        // 만약 단순 fold(접기)만 되어있고 spoiler/adult/비밀번호가 없으면 보여주고 싶다면 아래 줄을 삭제하세요.
        if (p.fold) return false;
  ...
  return (
    <div className="panel widget" style={{ margin: 0 }}>
      <h4>
        LATEST{" "}
        <span className="more" onClick={() => router.push("/gallery")}>
          더보기 ›
        </span>
      </h4>
...
function ddayLabel(
  date: string,
  plusOne?: boolean,
): { label: string; passed: boolean; near: boolean } {
  ...
  if (plusOne && diff <= 0)
    return { label: `D+${-diff + 1}`, passed: true, near: false };
      ...
      {items.map((it) => {
        ...
        return (
          <div className="dday-row" key={it.title}>
            <span>{it.title}</span>
            <b
              className={d.near && !dColor ? "d-red" : ""}
              style={{ fontFamily: familyOf(dFontId), color: dColor }}
            >
              {d.label}
            </b>
          </div>
  ...
  return (
    <div
      ...
      onClick={(e) => {
        ...
        if (
          (e.target as HTMLElement).closest(".k-check") ||
          (e.target as HTMLElement).closest(".modal-ov")
        )
          return;
        setOpen(true);
      }}
    >
      ...
      {items.map((it, i) => (
        <label
          className={`todo-row k-check ${it.done ? "done" : ""}`}
          key={`${it.text}-${i}`}
        ...
        >
          <input
            ...
            onChange={(ev) =>
              setItems(
                items.map((x, j) =>
                  j === i ? { ...x, done: ev.target.checked } : x,
                ),
              )
            }
          />
...
export function UpcomingWidget() {
  ...
  const seeSec = (secId?: string) =>
    canViewHref(menuSet, sectionHref("sched", secId ?? MAIN_SEC), viewer);
    ...
    .map((e) => {
      ...
      if (e.repeat === "yearly") {
        ...
        d =
          thisYear >= todayStr
            ? thisYear
            : `${today.getFullYear() + 1}-${e.start.slice(5)}`;
      }
      return { e, d };
    })
  ...
  return (
    ...
    >
      <h4>
        UPCOMING <span className="more">더보기 ›</span>
      </h4>
      {upcoming.map(({ e, d }) => (
        <div key={e.id} className="dday-row">
          <span>
            {d.slice(5).replace("-", ".")} · {e.title}
          </span>
          <b style={{ fontSize: 11, color: eventColor(e, st.cats) }}>●</b>
        </div>
      ))}
      {upcoming.length === 0 && (
        <p className="hint">다가오는 일정이 없습니다</p>
      )}
    </div>
  );
}
...
function ContainImg({
  ...
  const hitTest = (e: React.MouseEvent): boolean => {
    ...
    try {
      ...
      return (
        cacheRef.current.c.getContext("2d")!.getImageData(x, y, 1, 1).data[3] >
        8
      );
    } catch {
      return true;
  ...
  return (
    ...
    >
      ...
      <img
        ...
        draggable={false}
        onClick={(e) => {
          if (onActivate && hitTest(e)) onActivate();
        }}
        style={{
          maxWidth: "100%",
          maxHeight: "100%",
        ...
        fit === "contain" ? (
          ...
          <div
            style={{
              position: "absolute",
              inset: 0,
              cursor: canGo ? "var(--cur-pointer,pointer)" : undefined,
            }}
...
export function MemoBoardWidget() {
  ...
  const [menuSet] = useMenuSettings();
  if (
    !canViewHref(menuSet, "/memo", { loggedIn: !!user, isAdmin, id: user?.id })
  )
    return null;
  return (
    <div
...
export function ApplyWidget({ conf }: { conf: WidgetConf }) {
  ...
  if (
    !canViewHref(menuSet, "/comm-apply", {
      loggedIn: !!user,
      isAdmin,
      id: user?.id,
    })
    ...
    >
      ...
      {shown.map((a) => {
        ...
        return (
          <div className="apply-row" key={a.id}>
            ...
            {badge && (
              <i style={badgeStyle(badge, settings.badgeShape)}>
                {badge.label}
              </i>
            )}
          </div>
      ...
      >
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <KStep
            value={(conf.settings.count as number) ?? 5}
            min={1}
            max={20}
            suffix="명"
            onChange={(v) =>
              updateWidget(
                conf.id,
                { settings: { ...conf.settings, count: v } },
                { persist: true },
              )
            }
          />
