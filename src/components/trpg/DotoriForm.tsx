...
export function DotoriForm({
  ...
  const [status, setStatus] = useState<DotoriStatus>(
    initial?.status ?? "undecided",
  );
  ...
  return (
    <div className="write-grid">
      ...
      <div
        className="panel"
        style={{ padding: 24, display: "grid", gap: 12, alignContent: "start" }}
      >
        <label className="k-label" style={{ margin: 0 }}>
          이미지{" "}
          <span style={{ fontWeight: 400, color: "var(--faint)" }}>
            — 카드 썸네일 16:9 · 원본은 잘리지 않음 (선택)
          </span>
        </label>
        <div
          style={{
          ...
          onClick={() => document.getElementById("dtImgF")?.click()}
        >
          {fileUrl ? (
            ...
            <div className="ph" style={{ position: "absolute", inset: 0 }}>
              <span style={{ fontSize: 10 }}>SCENARIO</span>
            </div>
          )}
        </div>
        <input
      ...
      <div>
        <div className="panel widget" style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: "grid", gap: 9 }}>
            ...
            <KSelect
              value={status}
              onChange={(v) => setStatus(v as DotoriStatus)}
              options={DOTORI_STATUS_KEYS.map((s) => ({
                value: s,
                label: trpgSet.statuses[s].label,
              }))}
            />
            <p className="hint" style={{ margin: 0 }}>
              뱃지는 공수표·일정 확정만 카드에 표시 — 완은 완 탭에서만 보입니다
            </p>
          </div>
        </div>
        <div className="form-actions">
          <button className="btn btn-onbk" onClick={onCancel}>
            CANCEL
          </button>
      ...
      {cropOpen && (fileUrl || initial?.imgId) && (
        <DtCrop
          src={fileUrl}
          refId={!fileUrl ? initial?.imgId : undefined}
          crop={crop}
