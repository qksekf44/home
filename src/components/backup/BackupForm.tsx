"use client";
// 그림백업 작성/수정 공용 폼 (4.11) — 제목/유형/이미지 다중 업로드(원본·최적화·크롭·⠿순서)/설명/설정/접기/비밀글
...
export function BackupForm({ initial }: { initial: BackupPost | null }) {
  ...
  const [posts, setPosts] = useLocalList<BackupPost>(
    "ohome.backup.v1",
    BACKUP_SEED,
  );
  ...
  const tt = useSectionTitle(
    "gallery",
    initial ? (initial.secId ?? MAIN_SEC) : sec.id,
    isNew ? "WRITE" : "EDIT",
  );
  const [title, setTitle] = useState(initial?.title ?? "");
  const [type, setType] = useState<"log" | "single" | "vlist">(
    initial?.type ?? "log",
  );
  ...
  const [foldType, setFoldType] = useState<FoldType | "none">(
    initial?.fold?.type ?? "none",
  );
  const [foldLabel, setFoldLabel] = useState(initial?.fold?.label ?? "");
  // 비밀글 설정
  const [isSecret, setIsSecret] = useState(initial?.password ? true : false);
  ...
  if (
    isNew &&
    !canGalleryWrite(menuSet, sec.id, { loggedIn: true, isAdmin, id: user.id })
  ) {
    return (
      <section className="page">
        <div className="page-head">
          <PageTitle href={tt.href}>{tt.title}</PageTitle>
          <p>허용된 회원만 글을 쓸 수 있는 갤러리입니다</p>
        </div>
      </section>
    );
  }
  ...
  const post = async () => {
    ...
    if (isSecret) {
      if (!password.trim()) {
        toast("비밀글이 설정되었다면 비밀번호를 입력해 주세요");
        return;
      }
    }
    // 이미지 실저장 (IndexedDB) — 기존 ref는 유지, 새 파일만 저장
    const imageIds = await Promise.all(
      files.map((f) => (f.file ? putBlob(f.file) : Promise.resolve(f.ref!))),
    );
    if (isNew) {
      const p: BackupPost = {
        id: newId(),
        ...
        fold:
          foldType === "none"
            ...
            : {
                type: foldType,
                label: foldType === "custom" ? foldLabel : undefined,
              },
        // 비밀글 저장
  ...
  return (
    <section className="page">
      <div className="page-head">
        <PageTitle href={tt.href}>{tt.title}</PageTitle>
        <EditableDesc
          k={isNew ? "backup-write-desc" : "backup-edit-desc"}
          def={isNew ? "그림백업 글 작성" : "그림백업 글 수정"}
        />
      </div>
      <div className="write-grid">
        ...
        <div className="panel" style={{ padding: 24 }}>
          <div className="form-row">
            <label className="k-label" style={{ width: 60 }}>
              제목
            </label>
            <KInput
              value={title}
          ...
          <div className="form-row">
            ...
            <div style={{ display: "flex", gap: 18, alignItems: "center" }}>
              <KRadio
                ...
                value="log"
                current={type}
                onChange={(v) => setType(v as "log")}
                label={
                  <span>
                    로그 <span className="rd-desc">— 웹툰처럼 세로 스크롤</span>
                  </span>
                }
              />
              <KRadio
                ...
                value="single"
                current={type}
                onChange={(v) => setType(v as "single")}
                label={
                  <span>
                    단일{" "}
                    <span className="rd-desc">— 큰 이미지 + 좌우 넘김</span>
                  </span>
                }
              />
              <KRadio
                ...
                value="vlist"
                current={type}
                onChange={(v) => setType(v as "vlist")}
                label={
                  <span>
                    단일(세로){" "}
                    <span className="rd-desc">
                      — 이미지 사이 갭을 두고 세로로 나열
                    </span>
                  </span>
                }
          ...
          <div
            className="upzone"
            onClick={() => document.getElementById("bkFiles")?.click()}
          ...
          />
          {files.length > 0 && (
            <div className="upfile-count">
              ✓ {files.length}장 — 아래 순서대로 게시됩니다
            </div>
          )}
          <DragList
            items={files}
            ...
            render={(f, i) => (
              <div className="upfile-row" style={{ width: "100%" }}>
                ...
                <div className="nm">
                  <b>{f.name}</b>
                  <small>
                    {f.ref && !f.file ? "저장된 이미지" : fmtSize(f.size)}
                    {f.crop ? " · 썸네일 지정됨" : ""}
                  </small>
                </div>
                ...
                <div className="mini-seg">
                  <button
                    className={f.original ? "on" : ""}
                    onClick={() =>
                      setFiles((l) =>
                        l.map((x) =>
                          x.id === f.id ? { ...x, original: true } : x,
                        ),
                      )
                    }
                  >
        ...
        <div>
          <div className="panel widget" style={{ marginBottom: 14 }}>
            <h4>설정</h4>
            <div className="form-row">
              <label className="k-label" style={{ width: 70 }}>
                말머리
              </label>
              ...
              {isSecret && (
                <>
                  <div style={{ fontSize: 12, color: "#666", paddingLeft: 20 }}>
                    비밀번호로 보호된 글입니다. 열람할 때 비밀번호를 입력해야
                    합니다.
                  </div>
                  <div style={{ display: "grid", gap: 8, paddingLeft: 20 }}>
                    <div>
                      <label
                        className="k-label"
                    ...
                    {!isNew && (
                      <div
                        style={{ fontSize: 12, color: "#999", paddingTop: 4 }}
                      >
                        수정할 때는 기존 비밀번호를 변경할 수 있습니다.
                        (비워두면 기존 비밀번호 유지)
                      </div>
                    )}
                  </div>
            ...
            <div style={{ display: "grid", gap: 9 }}>
              ...
              {foldType === "custom" && (
                <KInput
                  placeholder="접기 문구"
                  value={foldLabel}
                  onChange={(e) => setFoldLabel(e.target.value)}
                />
              )}
            </div>
          </div>
          <div className="form-actions">
            <button
              ...
              onClick={() =>
                router.push(isNew ? tt.href : `/gallery/${initial.id}`)
              }
            >
              CANCEL
            </button>
      ...
      {cropFor && (
        <CropModal
          ...
          onApply={(c) => {
            setFiles((l) =>
              l.map((x) => (x.id === cropFor.id ? { ...x, crop: c } : x)),
            );
            setCropFor(null);
