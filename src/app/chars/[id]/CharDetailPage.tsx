...
function CharDetailInner() {
  ...
  const [chars, setChars, loaded] = useLocalList<Character>(
    "ohome.chars.v1",
    CHAR_SEED,
  );
  ...
  const charAus = useMemo(
    () =>
      ch
        ? rels.flatMap((r) =>
            r.members.some((m) => m.charId === ch.id)
              ? r.aus
                  .filter((a) => a.id !== "base")
                  .map((a) => ({
                    key: `${r.id}:${a.id}`,
  ...
  const pageColor =
    auRegistered && eff?.themeMode === "custom" ? eff.color : null;
  useEffect(() => {
  ...
  const editHref = auKey
    ? `/chars/${ch.id}/edit?au=${encodeURIComponent(auKey)}`
    : `/chars/${ch.id}/edit`;
  ...
  return (
    <section className="page page-char-detail">
      <div className="page-head">
        ...
        <EditableDesc
          k={`char-detail-desc:${ch.id}`}
          def="좌측 아이콘 탭 → 우측 정보 전환"
        />
      ...
      {charAus.length > 0 && (
        ...
        >
          <div
            className={`au-item ${auKey === null ? "on" : ""} ph ${ch.thumbClass}`}
            style={{
              borderColor: auKey === null ? "var(--accent)" : "var(--line)",
            }}
            onClick={() => setAuKey(null)}
          >
            {(ch.thumbId || ch.arts?.[0]) && (
              <CroppedBlobImg
                fileRef={ch.thumbId ?? ch.arts?.[0]}
                crop={ch.thumbCrop}
                ph={ch.thumbClass}
              />
            )}
          ...
          {charAus.map((a) => {
            ...
            return (
              <div
                key={a.key}
                className={`au-item ${auKey === a.key ? "on" : ""} ph ${ch.thumbClass}`}
                style={{
                  borderColor:
                    auKey === a.key ? "var(--accent)" : "var(--line)",
                }}
      ...
      {auKey && !auRegistered ? (
        <div className="panel" style={{ textAlign: "center", padding: 56 }}>
          ...
          <p style={{ fontSize: 13, color: "var(--faint)", marginBottom: 16 }}>
            이 AU의 「{ch.name}」이 아직 등록되지 않았습니다 — 등록하면 이
            캐릭터의 AU 프로필로 연동됩니다
          </p>
          {(isAdmin || charGrant(ch, user?.id) === "edit") && (
            <button
        ...
        <div className="profile-wrap">
          ...
          <div className="side-icons">
            <button
              className={tab === "basic" ? "on" : ""}
              data-tip="기본 정보"
              onClick={() => pickTab("basic")}
            >
              ☰
            </button>
            {eff.tabs.map((t) => (
          ...
          {(() => {
            ...
            if (arts.length === 0 && !eff.artUrl) {
              return (
                <div className={`profile-center ph ${ch.thumbClass}`}>
          ...
          <div
            className="panel profile-info"
            ref={infoRef}
            style={{ fontFamily: familyOf(eff.bodyFontId) }}
          >
            ...
            {tab === "basic" ? (
                      ...
                      <dd>
                        ...
                        >
                          ...
                          {eff.colors.map((c) => {
                            ...
                            const tip =
                              eff.colorTipMode === "label"
                                ? c.label || c.hex.toUpperCase()
                                : eff.colorTipMode === "both"
                                  ? c.label
                                    ? `${c.label} · ${c.hex.toUpperCase()}`
                                    : c.hex.toUpperCase()
                                  : c.hex.toUpperCase();
                            return (
                              <span
                                ...
                                style={{
                                  background: c.hex,
                                  boxShadow: chipBorder(eff.colorBd),
                                }}
                              />
                            );
                          })}
              ...
              <>
                ...
                {curTab?.subtitle && (
                  <div className="sub">{curTab.subtitle}</div>
                )}
                <div
                  className="prose"
...
export default function CharDetailPage({ id }: { id: string }) {
