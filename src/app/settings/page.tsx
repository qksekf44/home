...
import { useTrpgSettings } from "@/lib/trphStore";
...
function CP({
  ...
  return (
    <>
      ...
      <ColorField
        value={String(state.vars[k] ?? def ?? "#888888")}
        onChange={(hex) => setVar(k, hex as never)}
      />
    </>
  );
}
  ...
  return (
    <div className="set-row" style={{ flexWrap: "wrap" }}>
      <div className="l">
        <b>위젯</b>
        <small>
          메인·사이드에 얹히는 카드 — 비워 두면 카드 색을 그대로 따라갑니다
        </small>
      </div>
...
function FontRoleRow({ role }: { role: FontRole }) {
  ...
  const wBtn = (w: number | undefined, label: string) => (
    <button
      className={cfg.weight === w ? "on" : ""}
      onClick={() => setRole(role, { weight: w })}
    >
      {label}
    </button>
  );
  return (
    <div className="set-row" style={{ flexWrap: "wrap" }}>
      <div className="l">
        <b>{ROLE_LABEL[role].label}</b>
        {ROLE_LABEL[role].desc && <small>{ROLE_LABEL[role].desc}</small>}
      </div>
      <div
        style={{
          display: "flex",
      ...
      >
        ...
        <KSelect
          minWidth={170}
          value={cfg.id}
          onChange={(v) => setRole(role, { id: v })}
          options={[
            ...(role === "dropdown"
              ? [{ value: FOLLOW_MENU, label: <span>메뉴 폰트와 동일</span> }]
              : []),
            ...(role === "pagetitle"
              ? [
                  {
                    value: FOLLOW_TITLE,
                    label: <span>타이틀 폰트와 동일</span>,
                  },
                ]
              : []),
            ...fonts.map((f) => ({
...
function DesignPane() {
  ...
  const modeBtn = (m: typeof state.mode, label: string) => (
    <button className={state.mode === m ? "on" : ""} onClick={() => setMode(m)}>
      {label}
    </button>
  );
  return (
    <div className="set-sec">
      <h3>테마</h3>
      <div className="d">
        변경은 미리보기로만 반영 — [SAVE]를 눌러야 저장 · 모드마다 수정값을 따로
        기억합니다
      </div>
      ...
      <div className="set-row">
        <div className="l">
          <b>모드</b>
          <small>
            전환해도 각 모드의 수정값은 유지 — 초기화는 [선택 리셋]에서 모드별로
          </small>
        </div>
      ...
      <div className="set-row">
        <div className="l">
          <b>저장</b>
          <small>
            화면에는 바로 보이지만 <b>SAVE를 눌러야 실제로 저장</b>됩니다 —
            색·폰트·로고·탭 제목 모두
          </small>
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
      ...
      <div className="set-row" style={{ flexWrap: "wrap" }}>
        <div className="l">
          <b>테마 저장해두기</b>
          <small>
            현재 색 구성을 이름으로 보관 — 적용하면 커스텀 모드에 불러옵니다
          </small>
        </div>
        <div
          style={{
        ...
        >
          ...
          {presets.length > 0 && (
            <>
              ...
              <button
                className="btn btn-ghost"
                style={{ height: 35, padding: "0 14px", fontSize: 11 }}
                onClick={() => {
                  if (presetSel) {
                    applyPreset(presetSel);
                    toast("커스텀 모드에 적용했습니다 — 저장하려면 SAVE");
                  }
                }}
              >
                APPLY
      ...
      <div className="set-row">
        <div className="l">
          <b>포인트 자동 톤</b>
          <small>
            포인트 자동 모드를 다크 느낌으로 깔지, 라이트 느낌으로 깔지
          </small>
        </div>
        <div className="mini-seg">
          <button
            className={state.pointTone === "dark" ? "on" : ""}
            onClick={() => setPointTone("dark")}
          >
            다크 느낌
          </button>
          <button
            className={state.pointTone === "light" ? "on" : ""}
            onClick={() => setPointTone("light")}
          >
            라이트 느낌
          </button>
        </div>
      ...
      <div className="set-row">
        <div className="l">
          <b>포인트 컬러</b>
          <small>포인트 자동 모드의 기준색 — 변경 즉시 전체 재파생</small>
        </div>
        <div className="cp-group">
          <ColorField
            value={state.vars.accent}
            onChange={(hex) =>
              state.mode === "point"
                ? setPointAccent(hex)
                : setVar("accent", hex)
      ...
      <div className="set-row" style={{ flexWrap: "wrap" }}>
        <div className="l">
          <b>배경</b>
          <small>그라데이션(시작→끝·각도) 또는 이미지(업로드·블러)</small>
        </div>
        <div
          style={{
            display: "flex",
        ...
        >
          <div className="mini-seg">
            <button
              className={
                (state.vars.bgType ?? "gradient") === "gradient" ? "on" : ""
              }
              onClick={() => setVar("bgType", "gradient")}
            >
              그라데이션
            </button>
            <button
              className={state.vars.bgType === "image" ? "on" : ""}
              onClick={() => setVar("bgType", "image")}
            >
              이미지
            </button>
          </div>
            ...
            <>
              ...
              {state.vars.bgImageId && (
                <button
                  className="btn btn-ghost"
                  style={{ height: 35, padding: "0 14px", fontSize: 11 }}
                  onClick={() => setVar("bgImageId", undefined)}
                >
                  REMOVE
                </button>
              )}
      ...
      <div className="set-row">
        <div className="l">
          <b>카드</b>
          <small>
            패널·게시판 리스트·필터 카드의 배경과 글씨색 — 보조 글씨색은 자동
            파생
          </small>
        </div>
        <div className="cp-group">
      ...
      <div className="set-row" style={{ flexWrap: "wrap" }}>
        <div className="l">
          <b>브라우저 탭 제목</b>
          <small>
            탭·즐겨찾기에 표시되는 이름 — 비우면 「로고 텍스트 — 개인홈」
          </small>
        </div>
        <DocTitleControl />
      </div>
      ...
      <div className="set-row" style={{ flexWrap: "wrap" }}>
        <div className="l">
          <b>브라우저 탭 아이콘</b>
          <small>
            탭·즐겨찾기에 표시되는 작은 그림 — 정사각형 PNG 권장, 비우면 기본
            아이콘
          </small>
        </div>
        <FaviconControl />
      ...
      <div className="set-row" style={{ flexWrap: "wrap" }}>
        <div className="l">
          <b>크롤링 설명 문구</b>
          <small>
            카톡·디스코드 등에 링크 공유 시 제목 아래 뜨는 한 줄 — 비우면
            서브타이틀, 그것도 비었으면 기본 문구
          </small>
        </div>
        <CrawlDescControl />
      </div>
      ...
      <div className="set-row" style={{ flexWrap: "wrap" }}>
        <div className="l">
          <b>로고</b>
          <small>상단바 로고 텍스트·아랫줄 서브타이틀·정렬</small>
        </div>
        <div
          style={{
            display: "flex",
      ...
      <div className="set-row" style={{ flexWrap: "wrap" }}>
        <div className="l">
          <b>페이지 타이틀</b>
          <small>각 메뉴 상단 큰 제목과 설명 문구</small>
        </div>
        <div
          style={{
            display: "flex",
        ...
        >
          ...
          <div className="mini-seg">
            {(["both", "title", "desc", "none"] as const).map((m) => (
              <button
                key={m}
                className={(state.vars.pageHead ?? "both") === m ? "on" : ""}
          ...
          <div className="mini-seg">
            <button
              className={
                (state.vars.pageHeadM ?? "same") === "same" ? "on" : ""
              }
              onClick={() => setVar("pageHeadM", "same")}
            >
              동일
            </button>
            <button
              className={state.vars.pageHeadM === "none" ? "on" : ""}
              onClick={() => setVar("pageHeadM", "none")}
            >
              생략
            </button>
          </div>
      ...
      <div className="set-row" style={{ flexWrap: "wrap" }}>
        <div className="l">
          <b>입력 포커스</b>
          <small>인풋·드롭다운을 선택했을 때의 테두리색과 강조 링</small>
        </div>
        <div
          style={{
            display: "flex",
        ...
        >
          ...
          <div className="mini-seg">
            <button
              data-tip="은은하게 번지는 링"
              className={
                (state.vars.focusRing ?? "glow") === "glow" ? "on" : ""
              }
              onClick={() => setVar("focusRing", "glow")}
            >
              번짐
            </button>
            <button
              data-tip="선명한 실선 링"
              className={state.vars.focusRing === "line" ? "on" : ""}
              onClick={() => setVar("focusRing", "line")}
            >
              라인
            </button>
            <button
              data-tip="링 없이 테두리 색만 바뀜"
              className={state.vars.focusRing === "none" ? "on" : ""}
              onClick={() => setVar("focusRing", "none")}
            >
              테두리만
            </button>
          </div>
      ...
      <div className="set-row">
        <div className="l">
          <b>이미지 편집 배경</b>
          <small>썸네일·헤더 위치 지정 화면의 판 — 투명 이미지에서 보임</small>
        </div>
        <div className="cp-group">
      ...
      <div className="set-row">
        <div className="l">
          <b>스티커 메모 보드</b>
          <small>메모장 배치 판과 메인 미니보드 위젯의 배경</small>
        </div>
        <div className="cp-group">
      ...
      <div className="set-row">
        <div className="l">
          <b>검색창</b>
          <small>
            게시판·리스트 공통 검색 인풋 — 라이트/다크 전환 시 자동 연동
          </small>
        </div>
      ...
      <div className="set-row">
        <div className="l">
          <b>스위치 탭</b>
          <small>
            게시판 말머리·갤러리 보기 전환 등 — 고른 쪽은 아래 진한 버튼색을
            그대로 따라갑니다
          </small>
        </div>
        <div className="cp-group">
      ...
      <div className="set-row">
        <div className="l">
          <b>진한 버튼</b>
          <small>등록·저장 버튼과 체크박스·선택 필터 칩 공통</small>
        </div>
        <div className="cp-group">
      ...
      <div className="set-row">
        <div className="l">
          <b>모서리 둥글기</b>
          <small>카드/버튼 radius</small>
        </div>
        <KStep
          value={state.vars.radius}
          min={0}
          max={30}
          onChange={(v) => setVar("radius", v)}
        />
      </div>
      ...
      <div className="set-row">
        <div className="l">
          <b>블록 그림자</b>
          <small>
            패널·카드·배너·모달의 드롭섀도우 세기 — 0%는 그림자 없음
          </small>
        </div>
        ...
        >
          <span className="cp-lb" style={{ marginLeft: "auto" }}>
            색
          </span>
          <ColorField
            value={state.vars.shColor ?? "#000000"}
            onChange={(hex) => setVar("shColor", hex)}
          />
          <div className="mini-seg" style={{ marginLeft: 10 }}>
            {(
              ...
              <button
                key={label}
                className={state.vars.shadow === v ? "on" : ""}
                onClick={() => setVar("shadow", v)}
              >
                {label}
              </button>
            ))}
          ...
          <KStep
            value={state.vars.shadow}
            min={0}
            max={200}
            step={10}
            suffix="%"
            onChange={(v) => setVar("shadow", v)}
          />
        </div>
      </div>
      ...
      <div className="set-row">
        <div className="l">
          <b>드롭다운 그림자</b>
          <small>
            하위메뉴·프로필 메뉴·셀렉트 팝업·달력 — 블록 그림자와 별개, 없음도
            가능
          </small>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <div className="mini-seg">
            {(
              ...
              <button
                key={label}
                className={(state.vars.ddShadow ?? 100) === v ? "on" : ""}
                onClick={() => setVar("ddShadow", v)}
              >
                {label}
              </button>
            ))}
          ...
          <KStep
            value={state.vars.ddShadow ?? 100}
            min={0}
            max={200}
            step={10}
            suffix="%"
            onChange={(v) => setVar("ddShadow", v)}
          />
        </div>
      </div>
  ...
  return (
    <div className="set-row">
      <div className="l">
        <b>맞춤법 검사 밑줄</b>
        <small>
          입력칸·에디터에 브라우저가 그리는 빨간 물결줄 — 끄면 사이트 전체에서
          보이지 않습니다
        </small>
      </div>
      <div className="mini-seg">
        <button
          className={!off ? "on" : ""}
          onClick={() => set({ noSpell: false })}
        >
          표시
        </button>
        <button
          className={off ? "on" : ""}
          onClick={() => set({ noSpell: true })}
        >
          숨김
        </button>
      </div>
...
function MainPagePane() {
  ...
  return (
    <div className="set-sec">
      ...
      />
      <div className="d">
        모바일 표시 여부 + 모바일 세로 나열 순서 — ⠿ 드래그로 순서 변경
      </div>
      ...
      <DragList
        ...
        render={(w, i) => (
          <div className="set-row" style={{ width: "100%" }}>
            ...
            >
              ...
              <div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  {MULTI_TYPES.includes(w.type) && (
                    <span className="pill">중복 추가 가능</span>
                  )}
                </div>
            ...
            <KToggle
              checked={!w.mOff}
              onChange={(v) => setMobileOff(w.id, !v)}
            />
          </div>
        )}
      />
      <p className="hint">
        토글을 끄면 <b>모바일에서만</b> 빠지고 PC 메인에는 그대로 표시됩니다 ·
        순서는 <b>모바일 세로 나열 순서</b> — PC 배치는 메인에서 편집모드로 자유
        조정
      </p>
      <p className="hint">
        메인에서 위젯을 완전히 빼거나 더하려면 편집모드에서 — 추가는 상단바 [＋
        위젯], 삭제는 위젯 우클릭 → 위젯 삭제
      </p>
      <p className="hint">
        고정 요소: 슬라이드 배너(최상단) · 회원정보창 — 순서 목록에 포함되지
        않음
      </p>
    </div>
  );
...
function WidgetsPane() {
  ...
  const editable = state.widgets.filter((w) =>
    (["banner", "memo", "dday", "todo", "freetext", "deco"] as const).some(
      (t) => t === w.type,
    ),
  );
  ...
  const editorOf = (w: WidgetConf) => {
    switch (w.type) {
      case "memo":
      case "freetext":
        return <TextSettingEditor conf={w} />;
      case "dday":
        return <DdayEditor conf={w} />;
      case "todo":
        return <TodoEditor conf={w} />;
      case "deco":
        return <DecoEditor conf={w} />;
      case "banner":
        return <BannerEditor conf={w} />;
      default:
        return null;
    }
  };
  ...
  return (
    <div className="set-sec">
      <h3>위젯</h3>
      <div className="d">
        메인 위젯의 내용·설정값 관리 — 메인 페이지의 관리 모달과 같은 데이터라서
        어느 쪽에서 바꿔도 즉시 반영
      </div>
      ...
      {editable.map((w) => (
        ...
        >
          ...
          >
            ...
            <b style={{ fontSize: 13, letterSpacing: ".04em" }}>
              {widgetLabel(state.widgets, w)}
            </b>
            {MULTI_TYPES.includes(w.type) && (
              <span className="pill">중복 추가 가능</span>
            )}
            <small style={{ color: "var(--faint)", fontSize: 10.5 }}>
              {WIDGET_META[w.type].desc}
            </small>
            {w.mOff && (
      ...
      <p className="hint">
        DIARY·LATEST·UPCOMING은 일기·로드뷰·스케줄 실데이터를 그대로 보여주는
        위젯이라 별도 설정값이 없습니다 · 위젯 켜기/끄기와 순서는 「메인
        페이지」 카테고리에서
      </p>
    </div>
  );
...
function BoardPane() {
  ...
  const galSec = galSecs.some((s2) => s2.id === galSecSel)
    ? galSecSel
    : MAIN_SEC;
  ...
  const patchCat = (id: string, p: Partial<BoardBadge>) =>
    patchBoard(sel.id, {
      cats: sel.cats.map((c) => (c.id === id ? { ...c, ...p } : c)),
    });

  const colorCells = (
    b: BoardBadge,
    patch: (p: Partial<BoardBadge>) => void,
  ) => (
    <>
  ...
  return (
    <div className="set-sec">
      <h3>게시판 관리</h3>
      <div className="d">
        게시판 생성·삭제와 게시판별 스킨·권한·말머리 — 변경 즉시
        메뉴·목록·글쓰기에 반영
      </div>

      <h3 style={{ marginTop: 20 }}>게시판 목록</h3>
      <div className="d">
        ⠿ 드래그로 메뉴 순서 · 이름은 상단 메뉴와 페이지 타이틀에 그대로 표시 ·
        리스트 스킨(기본형/티켓형) 게시판마다 지정 — 글쓰기·댓글 권한은 메뉴
        관리에서
      </div>
      <DragList
        items={boards}
        ...
        render={(b) => (
          <div className="set-row" style={{ width: "100%" }}>
            ...
            >
              ...
              <KInput
                value={b.name}
                onChange={(e) => patchBoard(b.id, { name: e.target.value })}
                style={{ width: 110 }}
              />
            ...
            >
              ...
              <ColorField
                value={b.fg ?? "#2c3037"}
                onChange={(hex) => patchBoard(b.id, { fg: hex })}
              />
              {b.fg && (
                <button
                  className="btn btn-ghost"
                  style={{ padding: "4px 10px", fontSize: 10.5 }}
                  onClick={() => patchBoard(b.id, { fg: undefined })}
                >
                  기본색
                </button>
              )}
      ...
      <h3>말머리</h3>
      <div className="d">
        게시판을 고른 뒤 ⠿ 드래그로 순서 · 이름 수정 · 추가/삭제 — 이름을 바꿔도
        기존 글은 이전 이름으로 남습니다
      </div>
      {boards.length > 1 && (
        <div className="mini-seg" style={{ marginBottom: 12 }}>
          {boards.map((b) => (
            <button
              key={b.id}
              className={sel.id === b.id ? "on" : ""}
              onClick={() => setCatBoard(b.id)}
            >
              {b.name}
            </button>
          ))}
          ...
          <div className="set-row" style={{ width: "100%" }}>
            ...
            <div className="cp-group" style={{ justifyContent: "flex-end" }}>
              ...
              <button
                ...
                onClick={() =>
                  del.ask(
                    `말머리 「${c.label}」를 삭제하시겠습니까?`,
                    () =>
                      patchBoard(sel.id, {
                        cats: sel.cats.filter((x) => x.id !== c.id),
                      }),
                    "이 말머리로 쓴 기존 글은 유지되며 중립색 뱃지로 표시됩니다.",
                  )
                }
              >
                DELETE
      ...
      {st.system.map((b) => (
        <div key={b.id} className="set-row">
          <div className="l">
            <span style={boardBadgeStyle(b)}>{b.label}</span>
          </div>
          <div className="cp-group" style={{ justifyContent: "flex-end" }}>
      ...
      <h3>갤러리 유형 뱃지</h3>
      <div className="d">
        그림백업의 로그/단일 표시 — 라벨·색 수정 (삭제 불가)
      </div>
      {st.gallery.map((b) => (
        <div key={b.id} className="set-row">
          <div className="l">
            <span style={boardBadgeStyle(b)}>{b.label || "유형"}</span>
          </div>
          <div className="cp-group" style={{ justifyContent: "flex-end" }}>
            <KInput
              value={b.label}
              onChange={(e) => patchGallery(b.id, { label: e.target.value })}
              style={{ width: 90, textAlign: "right" }}
            />
      ...
      <div className="d">
        그림백업 글쓰기에서 고르는 말머리 — ⠿ 드래그로 순서 · 추가·수정·삭제
        자유
        {galSecs.length > 1 && (
          <>
            <br />
            갤러리마다 따로 정합니다 —{" "}
            <b>손대기 전까지는 기본 갤러리의 말머리를 그대로 씁니다</b>
          </>
        )}
      </div>
      {galSecs.length > 1 && (
        ...
        >
          {galSecs.map((s2) => (
            <button
              key={s2.id}
              className={galSec === s2.id ? "on" : ""}
              onClick={() => setGalSec(s2.id)}
            >
              {s2.name}
            </button>
          ))}
      ...
      <DragList
        items={galCats}
        keyOf={(c) => c.id}
        onReorder={(next) => setGalleryCats(galSec, next)}
        render={(c) => (
          <div className="set-row" style={{ width: "100%" }}>
            <div
              className="l"
            ...
            <div className="cp-group" style={{ justifyContent: "flex-end" }}>
              <KInput
                value={c.label}
                onChange={(e) =>
                  patchGalleryCat(galSec, c.id, { label: e.target.value })
                }
      ...
      <button
        className="btn btn-ghost"
        style={{ marginTop: 8, padding: "7px 14px", fontSize: 11 }}
        onClick={() => addGalleryCat(galSec)}
      >
        ＋ 말머리 추가
      </button>
...
function RelQPane() {
  ...
  return (
    <div className="set-sec">
      <h3>자관 질문</h3>
      <div className="d">
        질문 세트를 CP(커플)/NCP(커플 아님)로 구분해 관리 — 자관 상세에서
        QUESTIONS 섹션을 추가할 때 세트를 골라 넣습니다
      </div>
      ...
      <DragList
        ...
        render={(s) => (
          ...
          >
            <span className="drag-h" style={{ fontSize: 11 }}>
              ⠿
            </span>
            <KInput
              value={s.name}
              onChange={(e) => patchSet(s.id, { name: e.target.value })}
            ...
            <div className="mini-seg">
              {(["cp", "ncp"] as const).map((t) => (
                <button
                  key={t}
                  className={s.cat === t ? "on" : ""}
                  onClick={() => patchSet(s.id, { cat: t })}
                >
                  {CP_LABEL[t]}
                </button>
              ))}
            ...
            <button
              ...
              onClick={() =>
                del.ask(
                  `세트 「${s.name}」를 삭제하시겠습니까?`,
                  () => setSets(sets.filter((x) => x.id !== s.id)),
                  "이미 자관에 넣은 질문들은 그대로 유지됩니다.",
                )
              }
            >
              DELETE
      ...
      >
        <button
          ...
          onClick={() =>
            setSets([
              ...sets,
              { id: newId(), name: "새 세트", cat: "cp", questions: [] },
            ])
          }
        >
          ＋ ADD SET
              ...
              return (
                ...
                >
                  ...
                  <KInput
                    ...
                    onChange={(e) =>
                      patchSet(sel.id, {
                        questions: sel.questions.map((x, j) =>
                          j === gi ? e.target.value : x,
                        ),
                      })
                    }
                    style={{
                  ...
                  <span
                    ...
                    onClick={() => {
                      ...
                      const remove = () =>
                        patchSet(sel.id, {
                          questions: sel.questions.filter((_, j) => j !== gi),
                        });
                      if (q.trim())
                        del.ask(
                          `질문을 삭제하시겠습니까?`,
...
function MemberPane() {
  ...
  const authConsoleUrl = (() => {
    ...
    if (c?.kind === "firebase")
      return `https://console.firebase.google.com/project/${c.projectId}/authentication/users`;
    if (c?.kind === "supabase") {
      const m = c.url.match(/^https:\/\/([a-z0-9-]+)\.supabase\.co/i);
      return m
        ? `https://supabase.com/dashboard/project/${m[1]}/auth/users`
        : "";
    }
    return "";
  })();
  ...
  const registry = (() => {
    try {
      return JSON.parse(
        localStorage.getItem("ohome.mockreg.v1") ?? "{}",
  ...
  useEffect(() => {
    setMTags(getSetting<Record<string, string[]>>("ohome.membertags.v1", {}));
  }, []);
  const saveTags = (userId: string, tags: string[]) => {
    setMTags((prev) => {
      const n = { ...prev };
      if (tags.length) n[userId] = tags;
  ...
  const isAdminOf = (m: { id: string; role?: string }) =>
    m.role === "admin" || m.id === "admin";
  const filteredMembers = members
    ...
    .sort(
      (a, b) =>
        (isAdminOf(a) ? 0 : 1) - (isAdminOf(b) ? 0 : 1) ||
    ...
    <div className="set-sec">
      ...
      <div className="set-row" style={{ marginTop: 8, flexWrap: "wrap" }}>
        <div className="l">
          <b>가입코드</b>
          <small>
            회원가입 시 입력해야 하는 초대코드 — 아는 사람에게만 공유
          </small>
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <KInput
      ...
      <div className="d">
        기본 계정(관리자·지인회원)은 삭제할 수 없습니다 — 태그로 그룹화
      </div>
      <div className="set-row" style={{ flexWrap: "wrap" }}>
        <div className="l">
          {allTags.length > 0 ? (
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {allTags.map((t) => (
                <span
                  key={t}
                  className={`pill${filterTag === t ? " dark" : ""}`}
                  style={{ cursor: "var(--cur-pointer,pointer)" }}
                  onClick={() => {
                    setFilterTag((f) => (f === t ? null : t));
                    setMPage(1);
                  }}
                >
                  {t}
      ...
      {pageMembers.map((m) => {
        ...
        return (
          ...
          >
            ...
            <b
              style={{ fontSize: 12.5, cursor: "var(--cur-pointer,pointer)" }}
              data-tip="회원 정보 보기"
            ...
            <small
              ...
              data-tip={m.id.length > 7 ? m.id : undefined}
            >
            ...
            {!isBase && !isAdminOf(m) && (
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (serverOn2) {
                    setDelMember({ id: m.id, nickname: m.nickname });
                    return;
                  }
                  del.ask(
                    `회원 「${m.nickname}」 계정을 삭제하시겠습니까?`,
                    () => {
                      const reg = { ...registry };
                      delete reg[m.id];
                      try {
      ...
      {filteredMembers.length > PER_MEMBERS && (
        ...
        >
          <Pager
            page={mPage}
            total={Math.ceil(filteredMembers.length / PER_MEMBERS)}
            onChange={setMPage}
          />
        </div>
      )}
      ...
      <ConfirmModal
        open={delMember !== null}
        title={`회원 「${delMember?.nickname ?? ""}」 내보내기`}
        wide
        body={
          <div style={{ display: "grid", gap: 10 }}>
            <p style={{ margin: 0 }}>
              ...
              (그럴 수 있게 만들면 홈에 넣은 관리자 키가 공개돼 누구나 남의
              계정을 지울 수 있게 됩니다). 아래 버튼으로 콘솔을 열어{" "}
              <b>{delMember?.nickname}</b> 계정을 지워 주세요.
            </p>
            <p style={{ margin: 0 }}>
              <b>② 그다음 목록에서 지웁니다.</b>
              <br />
              계정을 지우지 않고 목록에서만 지우면{" "}
              <b>그 사람은 계속 로그인할 수 있습니다.</b>
            ...
            ? [
                {
                  ...
                  kind: "dark" as const,
                  onClick: () =>
                    window.open(authConsoleUrl, "_blank", "noopener"),
                },
              ]
            : []),
          {
            ...
            onClick: () => {
              ...
              void backend()
                ?.deleteMember(t.id)
                .then(() => {
                  setRemovedIds((v) => [...v, t.id]);
                  toast("회원 목록에서 지웠습니다");
                })
                .catch(() =>
                  toast(
                    "지우지 못했습니다 — 관리자 계정으로 로그인했는지 확인해 주세요",
                  ),
                );
            },
          },
  ...
  return (
    ...
    >
      <h3 style={{ color: "var(--accent)" }}>
        방문자에게는 아직 설치 화면이 보입니다
      </h3>
      <div className="d" style={{ lineHeight: 1.7 }}>
        데이터베이스 연결이 <b>지금 이 브라우저에만</b> 저장돼 있습니다 — 다른
        사람이 배포 주소로 들어오면 홈이 아니라 「처음 여는 중」 설치 화면이
        뜹니다. 아래 파일을 내려받아 <b>저장소의 public 폴더</b>에 올리고
        커밋하면(1~2분 뒤 자동 재배포) 방문자도 같은 홈을 봅니다. 공개용 값이라
        노출돼도 안전합니다.
        <br />
        적용됐는지는 <b>내 배포 주소/ohome.config.json</b>을 열어 값이 보이면
        성공입니다.
      </div>
      <div className="setup-row" style={{ marginTop: 10 }}>
        <button className="btn btn-dark" onClick={download}>
          ohome.config.json 내려받기
        </button>
      </div>
      <p className="hint" style={{ margin: "8px 0 0" }}>
        파일 대신 Vercel → Settings → Environment Variables에 환경변수로 넣는
        방법도 있습니다 — 설치 화면 마지막 단계와 같습니다.
      </p>
    </div>
  );
}
...
function ForkUpdateRow() {
  ...
  useEffect(() => {
    setUrl(getSetting<string>("ohome.repo.v1", ""));
    setLoaded(true);
  }, []);
  ...
  return (
    <div className="set-sec" style={{ marginTop: 26 }}>
      <h3>포크 업데이트</h3>
      <div className="d">
        내 포크 주소를 저장해 두면, 원본에 업데이트가 올라왔을 때 GitHub의 [Sync
        fork] 화면으로 바로 이동할 수 있습니다
      </div>
      <div className="set-row" style={{ flexWrap: "wrap" }}>
        <div className="l">
          <b>내 포크 주소</b>
          <small>GitHub에서 포크한 저장소의 주소</small>
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <KInput
            placeholder="https://github.com/내아이디/O.home"
            value={url}
          ...
          <button
            ...
            disabled={!loaded}
            onClick={() => {
              setSetting("ohome.repo.v1", trimmed);
              toast("저장되었습니다");
            }}
          >
            SAVE
          ...
          <button
            className="btn btn-dark"
            disabled={!valid}
            onClick={() => window.open(trimmed, "_blank", "noopener")}
          >
            내 포크로 이동 ↗
          </button>
        </div>
...
function SecurityRulesRow() {
  ...
  return (
    <div className="set-sec" style={{ marginTop: 26 }}>
      <h3>보안 규칙</h3>
      <div className="d">
        앱이 업데이트되며 규칙이 바뀔 때가 있습니다 — 새 기능이 갑자기 안
        보이거나 목록이 비어 보이거나 저장이 거부되면 아래를 다시 적용해 보세요.{" "}
        {cfg?.kind === "firebase"
      ...
      {cfg?.kind === "firebase" ? (
        <div className="setup-row">
          <button
            className="btn btn-dark"
            onClick={() => copy(FIRESTORE_RULES, "fs")}
          >
            {copied === "fs" ? "복사됨 ✓" : "Firestore 규칙 복사"}
          </button>
          <button
            className="btn btn-dark"
            onClick={() => copy(STORAGE_RULES, "st")}
          >
...
function MoodPane() {
  ...
  return (
    <div className="set-sec">
      <h3>무드 리스트</h3>
      <div className="d">
        다이어리에서 고를 무드 — 이름 · 아이콘(이모지/특수문자) · 색 · ⠿
        드래그로 순서
      </div>
      ...
      >
        <button
          ...
          onClick={() =>
            setMoods([
              ...moods,
              { id: newId(), name: "새 무드", icon: "✦", color: "#8a8f98" },
            ])
          }
        >
          ＋ ADD MOOD
...
function CrawlDescControl() {
  ...
  return (
    <LiveInput
      value={site.crawlDesc ?? ""}
      onValue={(v) => set({ crawlDesc: v })}
    ...
    >
      ...
      <input
        ...
        onChange={async (e) => {
          ...
          try {
            set({ favicon: await putBlob(f) });
          } catch (err) {
            toast(
              `아이콘을 저장소에 올리지 못했습니다 — ${err instanceof Error ? err.message : String(err)}`,
            );
          }
        }}
      />
...
function CursorRow({ state }: { state: CursorState }) {
  ...
  const setEntry = (p: Partial<{ imgId: string; hx: number; hy: number }>) =>
    patch({
      states: {
        ...st.states,
        [state]: { imgId: entry?.imgId ?? "", hx: 0, hy: 0, ...entry, ...p },
      },
    });
  return (
    <div className="set-row" style={{ flexWrap: "wrap" }}>
      ...
      >
        ...
        >
          {/* eslint-disable-next-line @next/next/no-img-element */}
          {url ? (
            <img src={url} alt="" style={{ maxWidth: 32, maxHeight: 32 }} />
          ) : (
            <span style={{ color: "var(--faint)", fontSize: 14 }}>✛</span>
          )}
        </span>
        <div>
          <b>{CURSOR_STATE_LABEL[state].label}</b>
          <small>{CURSOR_STATE_LABEL[state].desc}</small>
        </div>
      </div>
      <div
        style={{
      ...
      >
        <input
          id={inputId}
          type="file"
          accept="image/png,image/gif,image/webp,image/x-icon,.cur,.ani"
          style={{ display: "none" }}
          onChange={async (e) => {
            const f = e.target.files?.[0];
            e.target.value = "";
            ...
            if (resized)
              toast(
                "커서로 쓸 수 있게 128px 이내로 줄였습니다 (원본이 너무 컸음)",
              );
          }}
        />
        <button
          className="btn btn-ghost"
          style={{ height: 33, padding: "0 12px", fontSize: 11 }}
...
function CursorPane() {
  ...
  return (
    <div className="set-sec">
      <h3>마우스 커서</h3>
      <div className="d">
        상태별 커서 등록 (png·gif·cur·ani — 32px 내외 권장) + 클릭 지점(핫스팟)
        — ani는 애니메이션 재생 · cur/ani는 내장 핫스팟 사용 · 등록하지 않은
        상태는 기본 커서
      </div>
      <p className="hint" style={{ margin: "-6px 0 10px" }}>
        브라우저는 128px가 넘는 이미지를 커서로 쓰지 못합니다 — 큰 이미지를
        올리면 자동으로 줄여서 등록합니다
      </p>
      <div className="set-row">
        <div className="l">
          <b>커스텀 커서 사용</b>
          <small>끄면 전부 기본 커서로 (등록 이미지는 보존)</small>
        </div>
        <KToggle checked={st.enabled} onChange={(v) => patch({ enabled: v })} />
      </div>
      {(Object.keys(CURSOR_STATE_LABEL) as CursorState[]).map((s) => (
        <CursorRow key={s} state={s} />
      ))}
    </div>
  );
}
...
function DataPane() {
  ...
  const scanOrphans = async () => {
    ...
    try {
      ...
      toast(
        rows.length
          ? `쓰지 않는 이미지 ${rows.length}개를 찾았습니다`
          : "정리할 이미지가 없습니다",
      );
    } catch (e) {
  ...
  const [chars, setChars] = useLocalList<Character>(
    "ohome.chars.v1",
    CHAR_SEED,
  );
  ...
  const doPush = async () => {
    ...
    try {
      ...
      toast(
        n > 0
          ? `설정 ${n}건을 서버에 올렸습니다 — 방문자에게도 같은 모습으로 보입니다`
          : "올릴 설정이 없습니다",
      );
      setUnsynced(unsyncedSettingKeys());
    } catch {
      toast(
        "설정 업로드에 실패했습니다 — 관리자 계정으로 로그인했는지 확인해 주세요",
      );
    }
    setPushing(false);
  };
  const toggle = (k: string, v: boolean) =>
    setPicked((p) => (v ? [...new Set([...p, k])] : p.filter((x) => x !== k)));
  ...
  const migCfg = (): BackendConfig =>
    migKind === "firebase"
      ? {
          ...
          authDomain:
            migFb.authDomain.trim() ||
            `${migFb.projectId.trim()}.firebaseapp.com`,
          projectId: migFb.projectId.trim(),
          storageBucket:
            migFb.storageBucket.trim() ||
            `${migFb.projectId.trim()}.appspot.com`,
          appId: migFb.appId.trim(),
        }
      : {
          kind: "supabase",
          url: migSb.url.trim(),
          anonKey: migSb.anonKey.trim(),
        };
  ...
  const migCheck = async () => {
    ...
    try {
      ...
      setMigMsg(
        `연결에 실패했습니다 — ${(e as { message?: string })?.message ?? ""}`,
      );
      setMigState("idle");
    }
  };
  ...
  const migRun = async () => {
    ...
    try {
      ...
      setMigMsg(
        `이전 완료 — 항목 ${r.items}건 · 이미지 ${r.files}개. 아래 설정 파일을 저장소에 올리면 방문자도 새 DB를 봅니다.`,
      );
      setMigState("done");
    } catch (e) {
      setMigMsg(
        `이전에 실패했습니다 — ${(e as { message?: string })?.message ?? ""}`,
      );
      setMigState("ready");
    }
  };
  ...
  const migDownloadConfig = () => {
    const blob = new Blob([configFileText(migCfg())], {
  ...
  const doExport = async (includeMembers: boolean) => {
    ...
    try {
      ...
      URL.revokeObjectURL(u);
      toast(
        `백업 완료 — 데이터 ${dataCount}건 · 이미지 ${blobCount}개${includeMembers ? " · 회원 포함" : ""}`,
      );
    } catch {
      toast("백업 생성에 실패했습니다");
    }
  ...
  return (
    <div className="set-sec">
      <h3>데이터 백업</h3>
      <div className="d">
        글·캐릭터·자관·역극 등 모든 데이터(JSON)와 이미지 전부를 zip 하나로 —
        다른 브라우저/PC 이전용
      </div>
      ...
      {serverOn && unsynced.length > 0 && (
        <div className="set-row" style={{ flexWrap: "wrap" }}>
          <div className="l">
            <b>서버에 올리지 않은 설정 {unsynced.length}건</b>
            <small>
              서버를 붙이기 전에 이 브라우저에서 꾸민 설정입니다 — 올려야
              방문자에게도 같은 모습으로 보입니다
            </small>
          </div>
          <button
            className="btn btn-ghost"
            style={{ padding: "9px 18px", opacity: pushing ? 0.5 : 1 }}
            disabled={pushing}
            onClick={doPush}
          >
            {pushing ? "올리는 중…" : "↑ 설정 올리기"}
          </button>
        </div>
      )}
      ...
      {serverOn && (
        <div className="set-row" style={{ flexWrap: "wrap" }}>
          <div className="l">
            <b>쓰지 않는 이미지 정리</b>
            <small>
              글을 지워도 이미지는 저장소에 남습니다 — 어디에서도 쓰지 않는
              파일만 골라 지웁니다
            </small>
          </div>
          <div
          ...
          >
            {orphans && (
              <span className="hint">
                {orphans.length > 0
                  ? `${orphans.length}개 · ${orphanMB.toFixed(1)}MB`
                  : "정리할 것 없음"}
              </span>
            )}
            <button
              className="btn btn-ghost"
              style={{ padding: "9px 18px", opacity: scanning ? 0.5 : 1 }}
              disabled={scanning}
              onClick={scanOrphans}
            >
              {scanning ? "확인 중…" : "찾아보기"}
            ...
            {orphans && orphans.length > 0 && (
              <button
                ...
                style={{ padding: "9px 18px", opacity: scanning ? 0.5 : 1 }}
                disabled={scanning}
                onClick={() => setCleanAsk(true)}
              >
                {orphans.length}개 지우기
              </button>
            )}
          </div>
        </div>
      ...
      <div className="set-row" style={{ flexWrap: "wrap" }}>
        <div className="l">
          <b>등록되지 않은 상대 캐릭터 정리</b>
          <small>
            자관을 지워도 상대 캐릭터는 남습니다(실수로 지웠을 때를 위해) — 어느
            자관에도 없는 캐릭터만 골라 지웁니다
          </small>
        </div>
        <div
          style={{
        ...
        >
          ...
          {orphanChars.length > 0 && (
            <button
              ...
              onClick={() => setCharAsk(true)}
            >
              {orphanChars.length}명 지우기
            </button>
          )}
        </div>
      </div>
      ...
      <div className="set-row" style={{ flexWrap: "wrap" }}>
        <div className="l">
          <b>백업 내보내기</b>
          <small>
            글·캐릭터·설정 + 이미지 → zip · 회원 계정(가입자·가입코드) 포함 여부
            선택
          </small>
        </div>
        <div
        ...
        >
          <button
            className="btn btn-accent"
            style={{ padding: "9px 18px", opacity: busy ? 0.5 : 1 }}
            disabled={busy}
      ...
      {serverOn && backend()?.kind === "firebase" && (
        <p className="hint" style={{ margin: "10px 0 16px" }}>
          Firebase는 <b>저장소 CORS를 한 번 열어야</b> 백업 zip에 이미지가
          담깁니다 — 안 하면 글·설정만 담깁니다. 설치 가이드의 「백업」 항목에
          브라우저에서 끝내는 방법이 있습니다.
        </p>
      )}
      <div className="set-row" style={{ flexWrap: "wrap" }}>
        <div className="l">
          <b>백업 가져오기 (복원)</b>
          <small>
            현재 데이터를 백업 내용으로 덮어씁니다 — 복원 후 자동 새로고침
          </small>
        </div>
        <div
          style={{
      ...
      <div className="set-row" style={{ flexWrap: "wrap" }}>
        <div className="l">
          <b>데이터베이스 이전</b>
          <small>
            다른 프로젝트나 다른 서비스(Supabase ↔ Firebase)로 글·설정·이미지를
            통째로 옮깁니다
          </small>
        </div>
        <button
      ...
      <Modal
        ...
        actions={
          <>
            <button className="btn btn-ghost" onClick={() => setMigOpen(false)}>
              CLOSE
            </button>
            {migState === "ready" && (
              <button className="btn btn-accent" onClick={migRun}>
                이전 시작
              </button>
            )}
            {migState === "done" && (
              <button className="btn btn-accent" onClick={migSwitch}>
                새 DB로 전환
              </button>
            )}
          </>
      ...
      >
        <div className="mini-seg" style={{ marginBottom: 12 }}>
          <button
            className={migKind === "supabase" ? "on" : ""}
            onClick={() => setMigKind("supabase")}
          >
            Supabase
          </button>
          <button
            className={migKind === "firebase" ? "on" : ""}
            onClick={() => setMigKind("firebase")}
          >
            Firebase
          </button>
        </div>
        ...
        {migKind === "supabase" ? (
          <div style={{ display: "grid", gap: 8 }}>
            ...
            <KInput
              value={migSb.url}
              onChange={(e) => setMigSb((s) => ({ ...s, url: e.target.value }))}
            ...
            <KInput
              value={migSb.anonKey}
              onChange={(e) =>
                setMigSb((s) => ({ ...s, anonKey: e.target.value }))
              }
            />
            <p className="hint" style={{ margin: 0 }}>
              새 프로젝트라면 먼저 스키마 SQL을 실행해 두세요 — 설치 화면과 같은
              내용입니다.
            </p>
          </div>
        ) : (
          <div style={{ display: "grid", gap: 8 }}>
            ...
            >
              <div>
                ...
                <KInput
                  value={migFb.apiKey}
                  onChange={(e) =>
                    setMigFb((f) => ({ ...f, apiKey: e.target.value }))
                  }
                />
              </div>
              <div>
                ...
                <KInput
                  value={migFb.projectId}
                  onChange={(e) =>
                    setMigFb((f) => ({ ...f, projectId: e.target.value }))
                  }
                />
              </div>
            </div>
            <div>
              ...
              <KInput
                value={migFb.appId}
                onChange={(e) =>
                  setMigFb((f) => ({ ...f, appId: e.target.value }))
                }
              />
            </div>
            <p className="hint" style={{ margin: 0 }}>
              새 프로젝트라면 Firestore·Storage 보안 규칙을 먼저 붙여넣어
              두세요.
            </p>
          </div>
        )}
        ...
        >
          <button
            className="btn btn-dark"
            style={{ height: 33, padding: "0 16px", fontSize: 11 }}
            disabled={migState === "checking" || migState === "running"}
            onClick={migCheck}
          >
          ...
          {migState === "done" && (
            <button
              className="btn btn-ghost"
              style={{ height: 33, padding: "0 14px", fontSize: 11 }}
              onClick={migDownloadConfig}
            >
              ohome.config.json 내려받기
            </button>
          )}
      ...
      <div className="set-row">
        <div className="l">
          <b>초기화</b>
          <small>
            지울 항목을 골라서 — 메뉴별 데이터 / 사이트 설정 / 이미지 / 회원
            계정 (복구 불가, 백업을 먼저 내보내 두세요)
          </small>
        </div>
        <button
          className="btn btn-ghost"
          style={{ padding: "9px 20px", color: "var(--accent)" }}
          onClick={() => {
            setPicked([]);
            setResetOpen(true);
          }}
      ...
      >
        <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
          <button
            className="btn btn-ghost"
            style={{ padding: "5px 12px", fontSize: 11 }}
            onClick={() => setPicked(RESET_CONTENT.map((g) => g.key))}
          >
            메뉴 데이터 전체
          </button>
          <button
            ...
            style={{ padding: "5px 12px", fontSize: 11 }}
            onClick={() =>
              setPicked([...RESET_CONTENT, ...RESET_EXTRA].map((g) => g.key))
            }
          >
            전부 선택
          </button>
      ...
      <ConfirmModal
        open={cleanAsk}
        title={`쓰지 않는 이미지 ${orphans?.length ?? 0}개를 지울까요?`}
      ...
      <ConfirmModal
        open={charAsk}
        title={`상대 캐릭터 ${orphanChars.length}명을 지울까요?`}
        body={`${orphanChars
          .slice(0, 12)
          .map((c) => c.name)
          .join(
            ", ",
          )}${orphanChars.length > 12 ? ` 외 ${orphanChars.length - 12}명` : ""} — 어느 자관에도 등록돼 있지 않은 캐릭터입니다. 지우면 복구할 수 없습니다.`}
      ...
      <ConfirmModal
        open={resetAsk}
        title="선택한 항목을 초기화할까요?"
        body={`지울 항목: ${[...RESET_CONTENT, ...RESET_EXTRA]
          .filter((g) => picked.includes(g.key))
          .map((g) => g.label)
          .join(" · ")}\n삭제한 내용은 복구할 수 없습니다.`}
        onClose={() => setResetAsk(false)}
        buttons={[
            ...
            onClick: () => {
              ...
              void resetGroups(picked)
                .then((r) => {
                  ...
                  if (r.failed.length) {
                    toast(
                      `${r.failed.length}건을 서버에서 지우지 못했습니다 — 로그인·보안 규칙을 확인해 주세요`,
                    );
                  } else if (serverOn) {
                    ...
                    toast(
                      `서버에서 ${part.join(" · ")}를 지웠습니다${r.members ? " (로그인 계정은 콘솔에서 지워 주세요)" : ""}`,
                    );
                  } else {
                ...
                .catch(() => {
                  toast("초기화에 실패했습니다 — 로그인 상태를 확인해 주세요");
                })
                .finally(() =>
...
function MemberPickModal({
  ...
  const pool = useMembers().filter(
    (p) => p.id !== "admin" && p.role !== "admin",
  );
  ...
  return (
    <Modal
      ...
      desc={
        desc ??
        "체크한 회원(그리고 관리자)에게만 이 메뉴가 보이고 열립니다 — 비우면 로그인한 모든 회원"
      }
      actions={
        <>
    ...
    >
      <div style={{ display: "grid", gap: 9 }}>
        ...
        {sel.some((id) => !pool.some((p) => p.id === id)) && (
          <p className="hint" style={{ margin: 0 }}>
            회원 목록에 없는 선택{" "}
            {sel.filter((id) => !pool.some((p) => p.id === id)).length}건이
            있습니다 — 적용하면 그대로 유지됩니다
          </p>
        )}
      </div>
...
function MenuPane() {
  ...
  const [memAsk, setMemAsk] = useState<{
    ids: string[];
  ...
  useEffect(() => {
    ...
    if (!dirty) {
      setDraft(saved);
      setDraftRemoved(ms.removedBoards);
    }
  ...
  const saveAll = () => {
    patch({ tree, removedBoards: removed });
    toast("메뉴가 저장되었습니다");
  };
  ...
  const applyVis = async () => {
    ...
    if (!be) {
      toast(
        "서버에 연결돼 있지 않습니다 — 브라우저 저장 모드에는 서버 권한 자체가 없습니다",
      );
      return;
    }
    setVisBusy(true);
    try {
      ...
      for (const coll of CONTENT_COLLECTIONS) {
        ...
        if (targets.length)
          n += await be.refreshVis(coll, targets, user?.id ?? null);
      }
      toast(
        n
          ? `글 ${n}건을 서버에서도 가렸습니다`
      ...
      toast(
        e instanceof Error && e.message ? e.message : "적용하지 못했습니다",
      );
    }
    setVisBusy(false);
  };
  ...
  useEffect(() => {
    const onCapture = (e: MouseEvent) => {
      ...
      const hit = t?.closest?.(
        ".topbar a, .topbar button, .topbar .brand, .set-nav button",
      ) as HTMLElement | null;
      if (!hit) return;
      e.preventDefault();
      e.stopPropagation();
  ...
  useEffect(() => {
    setRpGate(
      getPageText("rp-gate-desc", "역극은 로그인한 참여자에게만 표시됩니다"),
    );
  }, []);
  ...
  useEffect(() => {
    ...
    let next: MenuGroupNode[] = saved
      .map((g) =>
        g.href
          ? menuLabelFor(g.href, extraAll) === null
            ? null
            : g
          : {
              ...g,
              items: g.items.filter(
                (it) => menuLabelFor(it.href, extraAll) !== null,
              ),
            },
      )
  ...
  const placedSet = new Set(
    tree.flatMap((g) => (g.href ? [g.href] : g.items.map((i) => i.href))),
  );
  const unplaced = [
    ...
    ...extraAll
      .map((b) => ({ href: b.href, label: b.name }))
  ...
  const askRemoveGroup = (g: MenuGroupNode) =>
    del.ask(
      g.href
        ? `단독 메뉴 「${g.label}」를 메뉴에서 빼시겠습니까?`
        : `상위 메뉴 「${g.label}」를 삭제하시겠습니까?`,
      () => removeGroup(g),
  ...
  const removeItem = (gid: string, href: string) => {
    setTree(
      tree.map((g) =>
        g.id === gid
          ? { ...g, items: g.items.filter((i) => i.href !== href) }
          : g,
      ),
    );
    markRemoved([href]);
  ...
  const moveItem = (fromGid: string, it: MenuLeaf, to: string) => {
    const stripped = tree.map((g) =>
      g.id === fromGid
        ? { ...g, items: g.items.filter((i) => i.href !== it.href) }
        : g,
    );
    if (to === "solo") {
      setTree([
        ...
        {
          id: newGroupId(),
          label: it.label ?? defLabel(it.href),
          href: it.href,
          items: [],
  ...
  const placeUnplaced = (href: string, to: string) => {
    if (to === "solo")
      setTree([
        ...tree,
        { id: newGroupId(), label: defLabel(href), href, items: [] },
      ]);
    else
      setTree(
        tree.map((g) =>
          g.id === to ? { ...g, items: [...g.items, { href }] } : g,
        ),
      );
    if (href.startsWith("/board?b="))
      setDraftRemoved(removed.filter((h) => h !== href));
  };
  const groupOptions = (excludeId?: string) => [
    ...tree
      .filter((g) => !g.href && g.id !== excludeId)
  ...
  const permSel = (
    label: string,
    value: MenuPerm,
    onChange: (v: MenuPerm) => void,
  ) => (
    <KSelect
      minWidth={110}
      value={value}
      onChange={(v) => onChange(v as MenuPerm)}
      options={[
    ...
    switch (href) {
      case "/gallery":
        return (
          <div className="mini-seg">
            <button
              className={ms.backupView === "gal" ? "on" : ""}
              onClick={() => patch({ backupView: "gal" })}
            >
              기본: 갤러리
            </button>
            <button
              className={ms.backupView === "list" ? "on" : ""}
              onClick={() => patch({ backupView: "list" })}
            >
              기본: 리스트
            </button>
          </div>
      ...
      case "/comm":
        return (
          <div className="mini-seg">
            <button
              className={commSet.ratio === "3:4" ? "on" : ""}
              onClick={() => patchComm({ ratio: "3:4" })}
            >
              비율 3:4
            </button>
            <button
              className={commSet.ratio === "4:3" ? "on" : ""}
              onClick={() => patchComm({ ratio: "4:3" })}
            >
              비율 4:3
            </button>
          </div>
      ...
      case "/cal":
        return (
          <div className="mini-seg">
            <button
              className={(ms.calTitle ?? "en") === "en" ? "on" : ""}
              onClick={() => patch({ calTitle: "en" })}
            >
              AUGUST 2026
            </button>
            <button
              className={ms.calTitle === "num" ? "on" : ""}
              onClick={() => patch({ calTitle: "num" })}
            >
              2026.08
            </button>
          </div>
  ...
  const extraPerm = (href: string) => {
    ...
    if (href === "/rp") {
      return (
        <KInput
          value={rpGate}
          onChange={(e) => {
            setRpGate(e.target.value);
            setPageText("rp-gate-desc", e.target.value);
          }}
          placeholder="비로그인 안내 문구"
    ...
    if (href === "/board" || href.startsWith("/board?b=")) {
      const bid =
        href === "/board" ? MAIN_BOARD_ID : href.slice("/board?b=".length);
    ...
    if (href === "/gallery" || href.startsWith("/gallery?s=")) {
      const key =
        href === "/gallery" ? MAIN_SEC : href.slice("/gallery?s=".length);
      // 메뉴 주소에는 별명(slug)이 적혀 있을 수 있다 — id로 통일해 저장
      const gsec = sectionsOf(secMap, "gallery").find(
        (s2) => s2.id === key || s2.slug === key,
      );
      if (!gsec) return null;
        ...
        <>
              ...
              onClick={() =>
                setMemAsk({
                  ...
                  apply: (n) => {
                    ...
                    if (n.length) next[gsec.id] = n;
                    else delete next[gsec.id];
          ...
          <KSelect
            ...
            onChange={(v) => {
              ...
              patch({
                galWrite: { ...ms.galWrite, [gsec.id]: v as MenuPerm },
                galWriteMembers: next,
              });
            }}
            options={[
              { value: "member", label: "글쓰기: 가입자" },
  ...
  const openChk = (
    ...
    (v ?? "all") === "all" ? null : (
      <KCheck
        label={<span style={{ fontSize: 11 }}>주소로는 열람 허용</span>}
        checked={!!open}
        onChange={(nv) => {
          if (nv) setOpenAsk(() => () => onChange(true));
          else onChange(false);
        }}
      />
    );
  ...
  const visSel = (v: MenuVis | undefined, onChange: (nv: MenuVis) => void) => (
    <KSelect
      minWidth={128}
      value={v ?? "all"}
      onChange={(nv) => onChange(nv as MenuVis)}
      options={[
  ...
  const memberBtn = (
    ...
    apply: (ids: string[] | undefined) => void,
  ) =>
    v !== "member" ? null : (
      <button
        className="btn btn-ghost"
        ...
        onClick={() =>
          setMemAsk({
            ids: ids ?? [],
            apply: (n) => apply(n.length ? n : undefined),
          })
        }
      >
  ...
  return (
    <div className="set-sec">
      ...
      <div className="mini-seg" style={{ marginBottom: 14 }}>
        <button
          className={mtab === "basic" ? "on" : ""}
          onClick={() => setMtab("basic")}
        >
          기본
        </button>
        <button
          className={mtab === "perm" ? "on" : ""}
          onClick={() => setMtab("perm")}
        >
          권한
        </button>
      </div>
      ...
      {mtab === "perm" ? (
        ...
        <div>
          <div className="d">
            공개범위는 메뉴 노출을 정합니다(전부 보임 / 비로그인 숨김 /
            관리자만) — SAVE를 눌러야 반영 · 글쓰기·댓글 권한은 즉시 반영
            <br />
            「비로그인 숨김」 옆 <b>멤버 선택</b>으로 특정 회원에게만 보이게
            좁힐 수 있습니다 — 비우면 로그인한 모든 회원
            <br />
            <b>주소로는 열람 허용</b>을 켜면 메뉴에서는 계속 감춰 두고{" "}
            <b>주소를 아는 사람만 들어올 수 있는</b> 메뉴가 됩니다 — 링크로 돌릴
            게시판에 씁니다
          </div>
          {tree.map((g) => (
            <div
            ...
            >
              ...
              >
                ...
                >
                  ...
                  {openChk(g.vis, g.open, (nv) =>
                    patchGroup(g.id, { open: nv || undefined }),
                  )}
                  {memberBtn(g.vis, g.visMembers, (ids) =>
                    patchGroup(g.id, { visMembers: ids }),
                  )}
                  {visSel(g.vis, (nv) =>
                    patchGroup(g.id, {
                ...
                g.items.map((it) => (
                  ...
                  >
                    ...
                    >
                      ...
                      {memberBtn(it.vis, it.visMembers, (ids) =>
                        patchGroup(g.id, {
                          items: g.items.map((x) =>
                            x.href === it.href ? { ...x, visMembers: ids } : x,
                          ),
                        }),
                      )}
                      {visSel(it.vis, (nv) =>
          ...
          >
            ...
            <div className="d" style={{ marginTop: 4, lineHeight: 1.7 }}>
              위 공개범위는 <b>화면에서 가리는 것</b>이라, 주소나 API를 직접
              부르면 글이 그대로 나옵니다. 이 버튼을 누르면{" "}
              <b>이미 올라간 글의 공개범위를 서버에 적용</b>해 서버가 아예
              내주지 않게 합니다 — 「비로그인 숨김」은 회원공개로,
              「관리자만」은 비공개로.
              <br />
              ...
              — 나중에 다시 공개로 바꾸려면 각 글의 공개범위를 직접 되돌려야
              합니다.
            </div>
            <div
              style={{
          ...
          <ConfirmModal
            open={!!openAsk}
            title="주소가 있는 모두에게 공개됩니다"
            body={
              "메뉴에서는 계속 감춰지지만, 이 주소를 아는 사람은 로그인하지 않아도 들어와 볼 수 있습니다. 링크를 받은 사람이 다른 곳에 옮겨 적으면 그 사람들도 볼 수 있습니다. 정말 알려지면 안 되는 내용에는 쓰지 마세요."
            }
          ...
          <ConfirmModal
            open={visAsk}
            title="글 공개범위를 서버에 적용할까요?"
            body={
              "비공개로 둔 메뉴의 글이 서버에서도 가려집니다. 각 글의 공개범위가 바뀌므로, 되돌리려면 글마다 직접 고쳐야 합니다."
            }
          ...
          <div style={{ marginTop: 18 }}>
            ...
            <div className="d" style={{ marginTop: 4 }}>
              체크한 영역에서 이미지 우클릭 저장·드래그 반출을 막습니다 — 관리자
              계정은 제외 · 즉시 반영
            </div>
            <div
              style={{
        ...
        <>
              ...
              >
                    ...
                    {g.href ? (
                      <>
                        ...
                        <KInput
                          ...
                          onChange={(e) =>
                            patchGroup(g.id, {
                              pageTitle: e.target.value || undefined,
                            })
                          }
                ...
                {!g.href && (
                  <div style={{ padding: "8px 0 0 34px" }}>
                        ...
                        >
                          ...
                          >
                            ...
                            <KSelect
                              ...
                              value=""
                              placeholder="이동"
                              onChange={(v) => moveItem(g.id, it, v)}
                              options={groupOptions(g.id)}
                            />
                            <button
                              className="btn btn-ghost"
                              style={{ padding: "3px 9px", fontSize: 10.5 }}
                              onClick={() => removeItem(g.id, it.href)}
                            >
                              빼기
                            </button>
                          </div>
                    ...
                    {g.items.length === 0 && (
                      <small style={{ color: "var(--faint)", fontSize: 10.5 }}>
                        하위 메뉴가 없으면 상단 메뉴에 표시되지 않습니다 — 아래
                        「미배치 기능」에서 추가
                      </small>
                    )}
                  </div>
          ...
          >
            <button
              ...
              onClick={() =>
                setTree([
                  ...tree,
                  { id: newGroupId(), label: "새 메뉴", items: [] },
                ])
              }
            >
              ＋ ADD MENU
          ...
          <div className="d">
            트리에 넣지 않은 기능 — 메뉴에 노출되지 않지만 데이터는 보존됩니다.
            넣을 위치를 고르면 바로 배치
          </div>
          {unplaced.map((f) => {
              ...
              <div key={f.href} className="set-row">
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <KSelect
                    ...
                    value=""
                    placeholder="배치할 위치"
                    onChange={(v) => placeUnplaced(f.href, v)}
                  ...
                  {link && (
                    <button
                      ...
                      onClick={() =>
                        del.ask(
                          `커스텀 링크 「${link.name}」를 지우시겠습니까?`,
                          () => setLinks(links.filter((x) => x.id !== link.id)),
                          "링크만 사라집니다 — 가리키던 페이지 자체는 그대로입니다.",
                        )
                      }
                    >
                      삭제
          ...
          {unplaced.length === 0 && (
            <div
              style={{ padding: "10px 0", fontSize: 12, color: "var(--faint)" }}
            >
              모든 기능이 메뉴에 배치되어 있습니다
            </div>
          )}
          ...
          <div className="d">
            자관·캐릭터처럼 목록에서 골라야 갈 수 있던 페이지를 메뉴에서 바로 —
            주소를 적어 두면 됩니다.
            <br />
            <b>내 홈 주소</b>는 붙여 넣으면 사이트 안 이동으로 바뀌고(예:{" "}
            <code>…/rels/latte</code> → <code>/rels/latte</code>),
            <b>다른 사이트 주소</b>는 그대로 남아 새 창으로 열립니다. 만든
            링크는 위 <b>미배치</b>에 나타나며, 거기서 원하는 상위 메뉴에 넣어
            주세요.
          </div>
          {links.map((l) => (
                ...
                <KInput
                  ...
                  onBlur={(e) =>
                    setLinks(
                      links.map((x) =>
                        x.id === l.id
                          ? { ...x, href: toInternalPath(e.target.value) }
                          : x,
                      ),
                    )
                  }
                    ...
                    () => {
                      ...
                      setTree(
                        tree
                          .map((g) => ({
                            ...g,
                            items: g.items.filter((i) => i.href !== l.href),
                          }))
                          .filter((g) => g.href !== l.href),
                      );
                    },
                    "메뉴에 배치해 두었다면 그 자리도 함께 비워집니다. 가리키던 페이지 자체는 그대로입니다.",
                  )
                }
              >
                DELETE
          ...
          {links.length === 0 && (
            <div
              style={{ padding: "10px 0", fontSize: 12, color: "var(--faint)" }}
            >
              아직 없습니다 — 아래에 이름과 주소를 적고 ADD를 눌러 주세요
            </div>
          )}
          ...
          >
            ...
            >
              ...
              <KInput
                value={nlHref}
                placeholder="/rels/latte 또는 풀주소"
                onChange={(e) => setNlHref(e.target.value)}
                style={{ width: 260 }}
              />
            </div>
            <button
              ...
              onClick={() => {
                if (!nlName.trim()) {
                  toast("메뉴에 보일 이름을 입력해 주세요");
                  return;
                }
                const href = toInternalPath(nlHref);
                if (!href || href === "/") {
                  toast("이동할 주소를 입력해 주세요");
                  return;
                }
                setLinks([
                  ...links,
                  { id: newId(), name: nlName.trim(), href },
                ]);
                setNlName("");
                setNlHref("");
                toast(
                  "링크가 만들어졌습니다 — 위 미배치에서 메뉴에 넣어 주세요",
                );
              }}
            >
              ＋ ADD
        ...
        buttons={[
          {
            label: "저장 후 이동",
            kind: "dark",
            onClick: () => leaveWith("save"),
          },
          {
            label: "저장하지 않고 이동",
            kind: "ghost",
            onClick: () => leaveWith("discard"),
          },
          {
            label: "CANCEL",
            kind: "ghost",
...
function TrpgPane() {
  ...
  useEffect(() => {
    setLockDesc(
      getPageText("trpg-lock-desc", "비밀번호를 입력하면 열람할 수 있습니다"),
    );
  }, []);
  const patchStatus = (
    k: DotoriStatus,
    p: Partial<(typeof settings.statuses)[DotoriStatus]>,
  ) =>
    patch({
      statuses: {
        ...settings.statuses,
        [k]: { ...settings.statuses[k], ...p },
      },
    });
  ...
  const colToggle = (list: string[], k: keyof MenuSettings) => (
    <div
      style={{
        flex: 1,
        display: "grid",
        gridTemplateColumns: `repeat(${PLAYLOG_COLS.length}, 1fr)`,
        gap: 6,
        justifyItems: "center",
      }}
    >
      {PLAYLOG_COLS.map((c) => (
        <KCheck
          key={c.key}
          label={c.label}
          checked={list.includes(c.key)}
          onChange={(v) =>
            patchMenu({
              [k]: v ? [...list, c.key] : list.filter((x) => x !== c.key),
  ...
  return (
    <div className="set-sec">
      ...
      <h3>로그 열람 안내 문구</h3>
      <div className="d">
        비밀번호를 건 로그에 들어갔을 때 보이는 문구입니다 — 관리자에게는 그
        화면이 뜨지 않아 여기서 고칩니다
      </div>
      <div className="set-row" style={{ alignItems: "center" }}>
        <div className="l">
          <b>비밀번호 안내</b>
          <small>비우면 기본 문구로 표시됩니다</small>
        </div>
        <KInput
          value={lockDesc}
          onChange={(e) => {
            setLockDesc(e.target.value);
            setPageText("trpg-lock-desc", e.target.value);
          }}
          placeholder="비밀번호를 입력하면 열람할 수 있습니다"
          style={{ width: 300 }}
        />
      </div>
      ...
      <h3>도토리 상태 카테고리</h3>
      <div className="d">
        라벨과 뱃지 색(배경/테두리/글씨) — 카드 뱃지는 공수표·일정 확정만 표시
      </div>
        ...
        return (
          <div key={k} className="set-row" style={{ alignItems: "center" }}>
            <div className="l">
              <span
                className="dt-badge"
                style={{ ...dotoriBadgeStyle(st), position: "static" }}
              >
                {st.label || "상태"}
              </span>
            </div>
            <div className="cp-group" style={{ justifyContent: "flex-end" }}>
              <KInput
                value={st.label}
                onChange={(e) => patchStatus(k, { label: e.target.value })}
                style={{ width: 100, textAlign: "right" }}
              />
              <span className="cp-lb">배경</span>
              <ColorField
                value={st.bg}
                onChange={(hex) => patchStatus(k, { bg: hex })}
              />
              <span className="cp-lb">테두리</span>
              <ColorField
                value={st.border}
                onChange={(hex) => patchStatus(k, { border: hex })}
              />
              <span className="cp-lb">글씨</span>
              <ColorField
                value={st.fg}
                onChange={(hex) => patchStatus(k, { fg: hex })}
              />
            </div>
          </div>
        );
      ...
      <h3>플레이기록 표시 열</h3>
      <div className="d">
        PC와 모바일에서 보여줄 열을 각각 선택 (기본: PC 전체 7열 · 모바일
        Date/Scenario/Role/Playtime)
      </div>
      {/* 라벨 폭 고정 — PC/모바일 폭 차이로 두 줄 체크박스 시작점이 어긋나던 것 정렬 (v1.9) */}
      <div className="set-row" style={{ flexWrap: "wrap" }}>
        <div className="l" style={{ width: 64, flexShrink: 0 }}>
          <b>PC</b>
        </div>
...
function MemoPane() {
  ...
  return (
    <div className="set-sec">
      <h3>스티커 메모장</h3>
      <div className="d">
        포스트잇 보드 옵션 — 배치·순서는 저장되어 모두에게 동일
      </div>
      <div className="set-row">
        <div className="l">
          <b>회원 작성 허용</b>
          <small>끄면 관리자만 메모를 붙일 수 있습니다</small>
        </div>
        <KToggle
          checked={settings.allowMember}
          onChange={(v) => patch({ allowMember: v })}
        />
      </div>
      <div className="set-row">
        <div className="l">
          <b>작성자 표시</b>
          <small>포스트잇 상단에 작성자 닉네임 표시</small>
        </div>
        <KToggle
...
function ThreadPane() {
  ...
  const setCats = (next: ThreadCat[]) =>
    patch(threadCatsPatch(settings, cur, next));
  ...
  return (
    <div className="set-sec">
      ...
      <div className="set-row">
        <div className="l">
          <b>첫 화면</b>
          <small>타래 보기 / 리스트 보기(포스터 카드 그리드)</small>
        </div>
        <div className="mini-seg">
          <button
            className={settings.defaultView === "thread" ? "on" : ""}
            onClick={() => patch({ defaultView: "thread" })}
          >
            타래 보기
          </button>
          <button
            className={settings.defaultView === "list" ? "on" : ""}
            onClick={() => patch({ defaultView: "list" })}
          >
            리스트 보기
          </button>
        </div>
      ...
      <div className="d">
        ...
        {secs.length > 1 && (
          <>
            <br />
            감상타래마다 따로 정합니다 —{" "}
            <b>손대기 전까지는 기본 감상타래의 분류를 그대로 씁니다</b>
          </>
        )}
      </div>
      {secs.length > 1 && (
      ...
      <DragList
        ...
        render={(c) => (
          <div className="set-row" style={{ width: "100%" }}>
            ...
            >
              <span className="drag-h">⠿</span>
              <span className="pill" style={threadBadgeStyle(c)}>
                {c.label || "분류"}
              </span>
            </div>
            <div className="cp-group" style={{ justifyContent: "flex-end" }}>
              ...
              <ColorField
                value={c.bg ?? "#1d2025"}
                onChange={(hex) => patchCat(c.id, { bg: hex })}
              />
              <span className="cp-lb">테두리</span>
              <ColorField
                value={c.border ?? c.bg ?? "#1d2025"}
                onChange={(hex) => patchCat(c.id, { border: hex })}
              />
              <span className="cp-lb">글씨</span>
              <ColorField
                value={c.fg ?? "#ffffff"}
                onChange={(hex) => patchCat(c.id, { fg: hex })}
              />
              <button
                className="btn btn-ghost"
                style={{ padding: "4px 10px", fontSize: 10.5 }}
                onClick={() => {
                  const used = works.filter(
                    (w) => w.catId === c.id && inSection(w.secId, cur),
                  ).length;
                  del.ask(
                    `분류 「${c.label}」를 삭제하시겠습니까?`,
                    () => setCats(cats.filter((x) => x.id !== c.id)),
                    used > 0
                      ? `이 분류의 타래 ${used}개는 유지되지만 분류가 「기타」로 표시됩니다.`
                      : undefined,
                  );
                }}
              >
    ...
    <>
      {list.map((b) => (
        <div key={b.id} className="set-row" style={{ alignItems: "center" }}>
          ...
          <div className="l">
            <span style={badgeStyle(b, shape)}>{b.label || "뱃지"}</span>
          </div>
          ...
          <div className="cp-group" style={{ justifyContent: "flex-end" }}>
            ...
            <ColorField
              value={b.bg}
              onChange={(hex) => patch(b.id, { bg: hex })}
            />
            <span className="cp-lb">테두리</span>
            <ColorField
              value={b.border}
              onChange={(hex) => patch(b.id, { border: hex })}
            />
            <span className="cp-lb">글씨</span>
            <ColorField
              value={b.fg}
              onChange={(hex) => patch(b.id, { fg: hex })}
            />
            {!fixedIds.includes(b.id) && (
              <button
                ...
                onClick={() =>
                  del.ask(
                    `뱃지 「${b.label}」를 삭제하시겠습니까?`,
                    () => onChange(list.filter((x) => x.id !== b.id)),
                    "이 뱃지를 쓰던 항목은 뱃지 없음으로 표시됩니다.",
                  )
                }
              >
                DELETE
...
function CommPane() {
  ...
  return (
    <div className="set-sec">
      <h3>커미션</h3>
      <div className="d">
        커미션 리스트·상세·신청자 리스트의 뱃지와 슬롯 설정 — 변경 즉시 반영
      </div>

      <div className="set-row">
        <div className="l">
          <b>썸네일 비율</b>
          <small>커미션 갤러리 전체에 일괄 적용</small>
        </div>
        <div className="mini-seg">
          <button
            className={st.ratio === "3:4" ? "on" : ""}
            onClick={() => patch({ ratio: "3:4" })}
          >
            3:4 세로
          </button>
          <button
            className={st.ratio === "4:3" ? "on" : ""}
            onClick={() => patch({ ratio: "4:3" })}
          >
            4:3 가로
          </button>
        </div>
      ...
      <div className="set-row">
        <div className="l">
          <b>뱃지 모양</b>
          <small>커미션·신청자 뱃지 공통</small>
        </div>
        <div className="mini-seg">
          <button
            className={st.badgeShape === "round" ? "on" : ""}
            onClick={() => patch({ badgeShape: "round" })}
          >
            라운드 스퀘어
          </button>
          <button
            className={st.badgeShape === "pill" ? "on" : ""}
            onClick={() => patch({ badgeShape: "pill" })}
          >
            동그란 필
          </button>
        </div>
      ...
      <div className="set-row">
        <div className="l">
          <b>전체 슬롯</b>
          <small>
            동시에 받을 수 있는 총 슬롯 — 리스트 상단에 남은/전체로 표시 (수동
            갱신)
          </small>
        </div>
        <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
          ...
          <KStep
            ...
            max={50}
            onChange={(v) =>
              patch({ totalSlot: v, totalUsed: Math.min(st.totalUsed, v) })
            }
          />
      ...
      <div className="set-row">
        <div className="l">
          <b>슬롯 표시 기준</b>
          <small>
            리스트·상세에 3/5로 적을 때 앞 숫자를 무엇으로 볼지 — 운영 방식에
            따라 다릅니다
          </small>
        </div>
        <div className="mini-seg">
          <button
            className={(st.slotDisplay ?? "used") === "used" ? "on" : ""}
            onClick={() => patch({ slotDisplay: "used" })}
          >
            채워진 슬롯
          </button>
          <button
            className={st.slotDisplay === "remain" ? "on" : ""}
            onClick={() => patch({ slotDisplay: "remain" })}
          >
            남은 슬롯
          </button>
        </div>
      ...
      <div className="set-row">
        <div className="l">
          <b>신청자 리스트 공개범위</b>
          <small>
            리스트 자체의 공개 — 각 신청 내용은 별개(관리자/허용된 본인만)
          </small>
        </div>
        <KSelect
          minWidth={130}
          value={st.applyVisibility}
          onChange={(v) =>
            patch({ applyVisibility: v as CommSettings["applyVisibility"] })
          }
          options={[
      ...
      <div className="set-row">
        <div className="l">
          <b>신청 휴지통 보관 기간</b>
          <small>
            휴지통으로 옮긴 신청을 며칠 두었다가 없앨지 — 기간이 지나면 목록을
            열 때 자동으로 사라집니다
          </small>
        </div>
        <KStep
          value={st.trashDays ?? 30}
          min={1}
          max={365}
          suffix="일"
          onChange={(v) => patch({ trashDays: v })}
        />
      </div>

      <h3 style={{ marginTop: 26 }}>커미션 뱃지</h3>
      <div className="d">
        기본 3종(모집중·마감·준비중)은 고정 제공 — 글씨·색은 자유 수정
      </div>
      <CommBadgeRows
        list={st.commBadges}
        shape={st.badgeShape}
        fixedIds={["open", "closed", "ready"]}
...
function BgmTrackRow({
  ...
  t: BgmTrack;
  onPatch: (p: Partial<BgmTrack>) => void;
  onDelete: () => void;
}) {
  const toast = useToast();
...
function BgmPane() {
  ...
  return (
    <div className="set-sec">
      <h3>BGM</h3>
      <div className="d">
        유튜브 곡 목록 관리 — 미니 플레이어 리스트에 노출 · 화면은 숨기고 소리만
        재생
      </div>
      ...
      <DragList
        ...
        render={(t) => (
          <BgmTrackRow
            ...
            onPatch={(p) =>
              setTracks(
                state.tracks.map((x) => (x.id === t.id ? { ...x, ...p } : x)),
              )
            }
            onDelete={() =>
              del.ask(`곡 「${t.title}」을 삭제하시겠습니까?`, () =>
                removeTrack(t.id),
              )
            }
          />
      ...
      <div className="set-row">
        <div className="l">
          <b>플레이어 위치</b>
          <small>기본: 오른쪽 아래</small>
        </div>
        <div className="mini-seg">
          <button
            className={state.settings.position === "br" ? "on" : ""}
            onClick={() => setSettings({ position: "br" })}
          >
            오른쪽 아래
          </button>
          <button
            className={state.settings.position === "bl" ? "on" : ""}
            onClick={() => setSettings({ position: "bl" })}
          >
            왼쪽 아래
          </button>
        </div>
      ...
      <div className="set-row">
        <div className="l">
          <b>반복 재생</b>
          <small>목록 끝에서 처음으로</small>
        </div>
        <KToggle
          checked={state.settings.repeat}
          onChange={(v) => setSettings({ repeat: v })}
        />
      </div>
      <div className="set-row">
        <div className="l">
          <b>플레이어 표시</b>
          <small>끄면 사이트에서 BGM 플레이어가 사라짐</small>
        </div>
        <KToggle
      ...
      <div className="set-row">
        <div className="l">
          <b>재생 방식</b>
          <small>
            자동: 접속 후 처음 클릭/키 입력하는 순간 재생 (브라우저 정책상 완전
            무동작 자동재생은 불가) · 수동: 재생 버튼을 눌러야 재생
          </small>
        </div>
        <div className="mini-seg">
          <button
            className={state.settings.autoplay ? "on" : ""}
...
function FontRow({ f }: { f: FontDef }) {
  const { fonts, familyOf, updateFont, removeFont, resetFont, setFontPair } =
    useFonts();
  const del = useConfirmDelete();
  const toast = useToast();
  const [editing, setEditing] = useState(false);
      ...
      >
        ...
        <div style={{ display: "flex", gap: 7 }}>
          ...
          <button
            ...
            style={{
              whiteSpace: "nowrap",
              marginLeft: f.fileId ? "auto" : undefined,
            }}
            onClick={() => {
              if (
                updateFont(f.id, {
                  name,
                  family: f.fileId ? f.family : family,
                  cssUrl: f.fileId ? "" : cssUrl,
                })
              ) {
                setEditing(false);
                toast("폰트가 수정되었습니다");
  ...
  return (
    <div className="set-row">
      <div className="l" style={{ minWidth: 0 }}>
        ...
        <b style={{ fontFamily: familyOf(f.id), fontSize: 15 }}>
          {f.name} — 가나다 ABC 123
        </b>
        <small
          style={{
...
function FontPane() {
  const { fonts, hiddenCount, addFont, addFontFile, restoreBuiltins } =
    useFonts();
  const toast = useToast();
  ...
  const uploadFontFile = async (f: File | undefined) => {
    ...
    if (await addFontFile(name, f, upPair || undefined)) {
      setUpPair("");
      toast(`「${name}」 폰트가 등록되었습니다`);
    }
  };
  ...
  return (
    <div className="set-sec">
      <h3>폰트 라이브러리</h3>
      <div className="d">
        캐릭터 프로필·자관 이름·시나리오 타이틀 등에서 이 목록 중 선택해 사용 —
        원하는 폰트만 남기고 삭제 가능
      </div>

      {fonts.map((f) => (
        <FontRow key={f.id} f={f} />
      ))}
      {hiddenCount > 0 && (
      ...
      <p className="hint">
        삭제해도 이미 그 폰트를 쓰고 있는 캐릭터·자관 표시는 깨지지 않습니다 —
        선택 목록에서만 빠집니다
      </p>

      <div style={{ display: "grid", gap: 8, marginTop: 14 }}>
        <label className="k-label" style={{ margin: 0 }}>
          웹폰트 추가 — 눈누/구글폰트의 CSS 링크(URL)와 font-family 값
        </label>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <KInput
            placeholder="CSS URL (https://fonts.googleapis.com/... 또는 눈누 링크)"
            value={cssUrl}
            onChange={(e) => setCssUrl(e.target.value)}
          />
          <button
            ...
            onClick={() => {
              if (
                addFont(
                  ...
                  family,
                  cssUrl,
                  webPairOn ? webPair || undefined : undefined,
                )
              ) {
                setName("");
                setFamily("");
      ...
      <div style={{ display: "grid", gap: 8, marginTop: 16 }}>
        ...
        >
          ...
          >
            ...
            <input
              ...
              style={{ display: "none" }}
              onChange={(e) => {
                uploadFontFile(e.target.files?.[0]);
                e.target.value = "";
              }}
            />
          </label>
        ...
        <p className="hint">
          표시 이름은 파일명 그대로 등록 — 목록의 [EDIT]에서 수정. 한글 미지원
          폰트라면 한글 페어를 지정 — 한글 글자는 페어 폰트로 표시됩니다.
        </p>
      </div>
    </div>
  );
...
function SettingsInner() {
  ...
  useEffect(() => {
    if (urlTab && (CATEGORIES as readonly string[]).includes(urlTab))
      setTabState(urlTab as (typeof CATEGORIES)[number]);
  ...
  useEffect(
    () => () => {
      if (themeRef.current.dirty) themeRef.current.discard();
    },
    [],
  );
  ...
  useEffect(() => {
    const onCapture = (e: MouseEvent) => {
      ...
      const t = e.target as HTMLElement | null;
      const hit = t?.closest?.(
        ".topbar a, .topbar button, .topbar .brand",
      ) as HTMLElement | null;
      if (!hit) return;
      e.preventDefault();
      e.stopPropagation();
  ...
  return (
    <section className="page">
      <div className="page-head">
        ...
        <EditableDesc
          k="settings-desc"
          def="노코드 커스터마이징 — 모든 컨트롤은 자체 UI 킷 (기본 브라우저 UI 미사용)"
        />
      </div>
