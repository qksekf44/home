...
const hasRichHtml = (html: string) =>
  /<(table|thead|tbody|tr|td|th|div|span|section|article|iframe|video|audio|details|summary|font|center)\b/i.test(
    html,
...
export default function IntroPage() {
  ...
  useEffect(() => {
    if (!editing) setDraft(doc.html);
  }, [doc.html, editing]);

  const start = () => {
    setDraft(doc.html);
    setMode(doc.mode ?? "editor");
    setEditing(true);
  };
  const done = () => {
    ...
    <section className="page">
            ...
            {editing ? (
              <>
                <div className="mini-seg" style={{ marginRight: 6 }}>
                  ...
                  <button
                    className={mode === "html" ? "on" : ""}
                    onClick={() => setMode("html")}
                  >
                    HTML
                  </button>
                </div>
                <button className="btn btn-dark" onClick={done}>
                  SAVE
                </button>
                <button
      ...
      <div className="panel" style={{ padding: 26 }}>
          ...
          mode === "html" ? (
            <>
              <KTextarea
                ...
                style={{
                  minHeight: 420,
                  fontFamily: 'var(--mono, Consolas, "D2Coding", monospace)',
                  fontSize: 12.5,
                  lineHeight: 1.7,
                }}
              />
              <p className="hint" style={{ margin: "8px 0 0" }}>
                태그를 직접 씁니다 — 보여 줄 때{" "}
                <b>스크립트·프레임은 걷어냅니다</b>(안전 때문). 이미지는 주소로
                넣거나 에디터 모드에서 올려 주세요.
              </p>
            </>
          ) : (
        ...
        loaded && !doc.html.trim() ? (
          <p
            className="hint"
            style={{ margin: 0, textAlign: "center", padding: "38px 0" }}
          >
            {isAdmin
              ? "아직 비어 있습니다 — 오른쪽 위 EDIT으로 소개를 작성해 주세요"
              : "아직 준비 중입니다"}
          </p>
        ) : (
          <HtmlBody
