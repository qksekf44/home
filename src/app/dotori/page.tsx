...
function DotoriPageInner() {
  ...
  const [itemsAll, setItemsAll, loaded] = useLocalList<DotoriItem>(
    "ohome.dotori.v1",
    DOTORI_SEED,
  );
  ...
  const TABS: { key: Tab; label: string }[] = [
    ...
    ...DOTORI_STATUS_KEYS.map((k) => ({
      key: k as Tab,
      label: trpgSet.statuses[k].label,
    })),
  ];
  ...
  const countOf = (t: Tab) =>
    items.filter((it) => (t === "all" ? it.status !== "done" : it.status === t))
      .length;
  ...
  const sort = useCardSort(
    shown,
    (next) => setItems(mergeOrder(items, next)),
    editOn && isAdmin,
  );
  ...
  const [ordFor, setOrdFor] = useState<{
    id: string;
    x: number;
    y: number;
  } | null>(null);
  ...
  const applyOrder = (wanted: number) => {
    if (ordIdx >= 0)
      setItems(mergeOrder(items, moveToOrder(shown, ordIdx, wanted)));
    setOrdFor(null);
  };
  ...
  return (
    <section className="page" onClick={() => setStatusFor(null)}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          {isAdmin && (
            <button
              ...
              onClick={() =>
                router.push("/dotori/new" + secQuery("dotori", sec.id))
              }
            >
              ＋ ADD
            </button>
      ...
      <div className="dt-grid">
        {shown.slice(start, start + PER_DT).map((it, si) => {
          ...
          return (
            <div
              key={it.id}
              className="panel dt-card"
              {...sort(i)}
              style={{
                cursor: isAdmin ? "pointer" : undefined,
                ...(sort(i) as { style?: React.CSSProperties }).style,
              }}
              onContextMenu={(e) => {
                if (!isAdmin) return;
                e.preventDefault();
              ...
              onClick={() => {
                if (isAdmin && !editOn) router.push(`/dotori/${it.id}/edit`);
              }}
            >
              <div className="th">
                <CroppedBlobImg
                ...
                {(it.status === "pledge" || it.status === "confirmed") && (
                  <span
                    className="dt-badge"
                    style={dotoriBadgeStyle(trpgSet.statuses[it.status])}
                  >
                ...
                {isAdmin && (
                  <div
                    className="hv-actions dt-actions"
                    onClick={(e) => e.stopPropagation()}
                  >
                    <button
                      onClick={() =>
                        setStatusFor((s) => (s === it.id ? null : it.id))
                      }
                    >
                      STATUS
                    </button>
                    <button className="del" onClick={() => setDelFor(it)}>
                      DELETE
                    </button>
                  </div>
                )}
                ...
                {statusFor === it.id && (
                  <div
                    className="dt-status-pop"
                    onClick={(e) => e.stopPropagation()}
                  >
                    {DOTORI_STATUS_KEYS.map((s) => (
                      <button
                        key={s}
                        className={it.status === s ? "on" : ""}
                        onClick={() => {
                          setStatus(it.id, s);
                          setStatusFor(null);
                        }}
      ...
      {ordFor && ordIdx >= 0 && (
        <OrderMenu
          at={ordFor}
          current={orderNoOf(ordIdx)}
          total={shown.length}
          onApply={applyOrder}
          onClose={() => setOrdFor(null)}
        />
      )}
        ...
        buttons={[
          {
            ...
            kind: "accent",
            onClick: () => {
              setItems(items.filter((x) => x.id !== delFor!.id));
              setDelFor(null);
            },
          },
