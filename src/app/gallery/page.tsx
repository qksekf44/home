...
function BackupPageInner() {
  ...
  const [postsAll, setPostsAll] = useLocalList<BackupPost>(
    "ohome.backup.v1",
    BACKUP_SEED,
  );
  ...
  const sort = useCardSort(
    visible,
    (next) => setPosts(mergeOrder(posts, next)),
    editOn && isAdmin,
  );
  ...
  return (
    <section className="page">
      <div className="page-head">
        ...
        <EditableDesc
          k="backup-desc"
          def="로그형(웹툰 스크롤) / 단일형(좌우 넘김) · 리스트/갤러리 보기 전환"
        />
      </div>
      <div className="toolrow">
        <div className="seg">
          <button
            className={view === "gal" ? "on" : ""}
            onClick={() => setView("gal")}
          >
            갤러리
          </button>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          {canGalleryWrite(menuSet, sec.id, {
            loggedIn: !!user,
            isAdmin,
            id: user?.id,
          }) && (
            <button
              ...
              onClick={() =>
                router.push("/gallery/write" + secQuery("gallery", sec.id))
              }
            >
              ✎ WRITE
            </button>
      ...
      <div
        ...
        style={{
          display: view === "list" && visible.length > 0 ? undefined : "none",
        }}
      >
        {paged.map((p) => {
