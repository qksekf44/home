...
export default function BackupDetailPage() {
  ...
  useEffect(() => {
    ...
    if (p.desc) {
      const cleanDesc = p.desc.replace(/<[^>]*>?/gm, "").slice(0, 160);
      updateMetaTag("property", "og:description", cleanDesc);
      updateMetaTag("name", "description", cleanDesc);
    }

    // p.images.length > 0이면 p.images[0]으로 og:image 설정
    if (p.images && p.images.length > 0) {
      updateMetaTag("property", "og:image", p.images[0]);
      updateMetaTag("name", "twitter:image", p.images[0]);
