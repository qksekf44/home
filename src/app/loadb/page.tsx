...
function RoadBlock({
  ...
  const saveEdit = () => {
    if (editCid && editText.trim())
      onEditComment(item.id, editCid, editText.trim());
    setEditCid(null);
  };
  const post = () => {
    if (!text.trim()) return;
    if (guestMode && !gName.trim()) {
      toast("닉네임을 입력해 주세요");
      return;
    }
    onComment(
      ...
      >
        {imgSrc ? (
          ...
          <img
            src={imgSrc}
            alt={item.title}
            className={`artimg ${item.narrow ? "narrow" : ""}`}
            style={{ filter: folded ? "blur(18px)" : undefined }}
          />
        ) : (
          <div
            className={`artimg ${item.narrow ? "narrow" : ""} ph ${item.ph}`}
            style={{
              aspectRatio: item.ratio,
              filter: folded ? "blur(18px)" : undefined,
            }}
          >
        ...
        {folded && (
          <div className="cover" onClick={() => setOpen(true)}>
            <div>
              <b>
                {item.fold!.type === "custom"
                  ? item.fold!.label || "접힘"
                  : FOLD_LABEL[item.fold!.type]}
              </b>
              <br />
              <span>클릭하여 표시</span>
            </div>
      ...
      <div className="cmt-side">
        ...
        <div className={`cmt-input ${guestMode && canComment ? "guest" : ""}`}>
          ...
          <div
            className="ci-row"
            style={
              guestMode && canComment ? undefined : { display: "contents" }
            }
          >
            <KInput
              placeholder={
...
function RoadviewPageInner() {
  ...
  const allow = (p: MenuPerm) =>
    p === "admin" ? isAdmin : p === "member" ? !!user : true;
  const [itemsAll, setItemsAll, roadLoaded] = useLocalList<RoadItem>(
    "ohome.road.v1",
    ROAD_SEED,
  );
  ...
  const [cmtRows, setCmtRows] = useLocalList<CommentRow>(
    COMMENT_KEY,
    COMMENT_SEED,
  );
  const [q, setQ] = useState("");
  const [page, setPage] = useState(1); // 인피니티 스크롤 → 페이지네이션
  ...
  const editLevel = (c: Comment): "free" | null =>
    user && c.authorId === user.id ? "free" : null;
  const delLevel = (c: Comment): "free" | null =>
    isAdmin || (user && c.authorId === user.id) ? "free" : null;

  const visible = items.filter(
    (it) =>
      !q ||
      it.author.includes(q) ||
      padNo(it.no).includes(q) ||
    ...
    <section className="page loadb-container">
      ...
      {pageItems.map((it) => (
        <RoadBlock
          key={it.id}
          item={it}
          comments={commentsFor(cmtRows, "road", it.id, it.comments)}
          onComment={addComment}
          onEditComment={editComment}
          onDeleteComment={deleteComment}
          canComment={
            allow(menuSet.roadComment) &&
            (!!user || menuSet.roadComment === "guest")
          }
                ...
                setItems(
                  items.map((x) =>
                    x.id === editFor!.id
                      ? {
                          ...x,
                          no: Number.isFinite(nv) && nv > 0 ? nv : x.no,
      ...
      >
        <div style={{ display: "grid", gap: 9 }}>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <KInput
              value={eNo}
              onChange={(e) => setENo(e.target.value.replace(/[^\d]/g, ""))}
              style={{ width: 90, textAlign: "center" }}
            />
          </div>
          <KCheck
            label="수위 주의 접기 (블러 + 클릭 표시)"
            checked={eAdult}
            onChange={setEAdult}
          />
        </div>
        ...
        buttons={[
          {
            ...
            onClick: () => {
              ...
              setCmtRows(
                cmtRows.filter(
                  (c) => !(c.target === "road" && c.targetId === gone),
                ),
              );
              setDelFor(null);
            },
