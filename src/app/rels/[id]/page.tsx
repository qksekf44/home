...
function CharFace({
  ...
  if (!rep)
    return (
      <div
        className={`${className ?? ""} ph ${c?.thumbClass ?? ""}`}
        style={style}
      />
    );
  return (
...
const noteOf = (m: RelMember) =>
  m.linkedNote === "상대 캐릭터" ? "" : (m.linkedNote ?? "");
...
function MiniProf({
    ...
    return (
      ...
      >
        ...
        <p className="hint" style={{ marginTop: 10 }}>
          이 AU의 프로필이 아직 등록되지 않았습니다
          <br />
          카드를 누르면 캐릭터 페이지에서 등록할 수 있습니다
        </p>
      </div>
    );
  }
  return (
    ...
    >
      <div className="hd">
        {rep ? (
          ...
          >
            ...
            <CroppedBlobImg
              fileRef={rep}
              crop={member.faceCrop ?? char.thumbCrop}
            />
          </div>
        ) : (
        ...
        <div>
          ...
          <b
            style={{
              fontFamily: familyOf(char.fontId),
              fontSize: member.nameSize ?? undefined,
          ...
          <small>
            {[char.sub, noteOf(member)].filter(Boolean).join(" · ")}
          </small>
        </div>
      </div>
      <div className="specs">
      ...
      {lb !== null && (
        <Lightbox srcs={gallery} index={lb} onClose={() => setLb(null)} />
      )}
  ...
  return (
    ...
    >
      ...
      >
        {isAdmin ? (
          <>
            <b style={{ fontSize: 20, display: "block", marginBottom: 6 }}>
              ＋
            </b>
            멤버 추가
            <br />
            <small>내 캐릭터 또는 상대 캐릭터</small>
          </>
        ) : (
          "멤버 미정"
        )}
...
export default function RelDetailPage() {
  ...
  const [rels, setRels, loaded] = useLocalList<Relation>(
    "ohome.rels.v1",
    REL_SEED,
  );
  const [chars, setChars, charsLoaded] = useLocalList<Character>(
    "ohome.chars.v1",
    CHAR_SEED,
  );
  ...
  const [faceEdit, setFaceEdit] = useState<{
    charId: string;
    ref: string;
    crop?: CropValue;
  } | null>(null);
  // 타임라인 항목 우클릭 메뉴 (v2.0 사용자 요청) — 수정·삭제. 늘 떠 있는 [삭제] 글자는 없앴다
  const [tlCtx, setTlCtx] = useState<{
    x: number;
    y: number;
    idx: number;
  } | null>(null);
  ...
  const [ansCtx, setAnsCtx] = useState<{
    x: number;
    y: number;
    idx: number;
  } | null>(null);
  useEffect(() => {
    ...
    const key = (e: KeyboardEvent) => {
      if (e.key === "Escape") setAnsCtx(null);
    };
    window.addEventListener("click", close);
    window.addEventListener("scroll", close, true);
    window.addEventListener("keydown", key);
  ...
  const [qaPickPos, setQaPickPos] = useState<{
    left: number;
    top: number;
  } | null>(null);
  // 관리자 추가 모달
  ...
  const [tSays, setTSays] = useState<
    { id: string; charId: string; text: string }[]
  >([]);
  ...
  const pageColor =
    effThemeMode === "custom" && effThemeColor ? effThemeColor : null;
  ...
  const themeAuStyle = rel
    ? auStyle(
        rel,
        rel.aus.find((a) => a.id === auId),
      )
    : undefined;
  ...
  useEffect(() => {
    ...
    const alive = rel.members.filter((m) =>
      chars.some((c) => c.id === m.charId),
    );
    if (alive.length !== rel.members.length) {
      setRels(
        rels.map((r) => (r.id === rel.id ? { ...r, members: alive } : r)),
      );
    }
  ...
  const saveArtCrop = (ref: string, c: CropValue | undefined) =>
    updateRel({
      artCrops: (() => {
  ...
  const qaOn =
    (isBaseAu ? rel?.qaEnabled : au?.qaEnabled) ?? auQuestions.length > 0;
  const curQa: QaEntry | undefined = auQuestions.find(
    (q) => q.no === (qaNo ?? auQuestions[0]?.no),
  );
  ...
  const answersOf = (no: number): MergedAnswer[] =>
    answersFor(
      ...
      au?.id ?? "base",
      no,
      auQuestions.find((q) => q.no === no)?.answers ?? [],
    );
  ...
  const qaHidden =
    !!rel?.qaHide && !isAdmin && !hasRelGrant(rel.members, chars, user?.id);
  ...
  const fullRefOf = (cid: string) =>
    isBaseAu
      ? rel?.members.find((x) => x.charId === cid)?.fullImgId
      : au?.fulls?.[cid];
  ...
  const auUnregOf = (cid: string) =>
    !!auCharKey && !findChar(chars, cid)?.auProfiles?.[auCharKey];
  ...
  const sideOf = (cid: string) =>
    isDuo && rel?.members[1]?.charId === cid ? "r" : "l";

  // AU 전환으로 QUESTIONS 섹션이 없는 AU에 오면 타임라인 탭으로 (v1.9)
  useEffect(() => {
    if (!qaOn && tab === "qa") setTab("tl");
  }, [qaOn, tab]);
  ...
  const qaFiltered = useMemo(
    () =>
      auQuestions.filter(
        (q) =>
          !qaQuery || q.q.includes(qaQuery) || String(q.no).includes(qaQuery),
      ),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [rel, auId, qaQuery],
  );
  const relLogs = useMemo(
    () => logs.filter((l) => l.relId === rel?.id),
    [logs, rel],
  );
  // 역극 연동 (4.9) — 내가 참여한 방 + 공개 전환된 완결 방만 (비참여 방은 존재 자체 비노출)
  const relRooms = useMemo(
    () =>
      rooms.filter(
        (rm) =>
          rm.relId === rel?.id &&
          ((user && rm.memberIds.includes(user.id)) ||
            (rm.status === "done" && rm.isPublic)),
      ),
    [rooms, rel, user],
  );
  ...
  if (
    !rel ||
    (rel.visibility === "private" && !isAdmin) ||
    (rel.visibility === "member" && !user)
  ) {
    return (
      <section className="page">
        <div className="page-head">
          <PageTitle>RELATIONS</PageTitle>
          <p>자관을 찾을 수 없거나 열람 권한이 없습니다</p>
        </div>
      </section>
    );
  }
  ...
  const patchAuData = (p: {
    ...
    if (isBaseAu) updateRel(p);
      ...
      updateRel({
        aus: rel.aus.map((a) => (a.id === au!.id ? { ...a, ...p } : a)),
      });
  };

  /* ---------- 멤버 추가 (내 캐릭터 or 새 상대 캐릭터) ---------- */
  const candidates = chars.filter(
    (c) => !rel.members.some((m) => m.charId === c.id),
  );
  const addMember = () => {
    let cid = mCharId;
    if (mMode === "new") {
      if (!mName.trim()) {
        toast("상대 캐릭터 이름을 입력해 주세요");
        return;
      }
  ...
  const addTlItem = () => {
    const says = tSays
      .filter((x) => x.charId && x.text.trim())
      .map(({ charId, text }) => ({ charId, text: text.trim() }));
    if (!tDesc.trim() && says.length === 0) {
      toast("설명 또는 한마디 중 하나는 입력해 주세요");
      return;
    }
    const item: TlItem = {
  ...
  const openTlEdit = (i: number) => {
    ...
    setTDesc(it.desc ?? "");
    setTSays(
      it.says.map((sy) => ({ id: newId(), charId: sy.charId, text: sy.text })),
    );
    setTlEditIdx(i);
    setTlOpen(true);
  };
  ...
  const addQuestionSet = (set: RelQuestionSet | null) => {
    ...
    patchAuData({
      qaPool: [...auQaPool, ...fresh],
      questions: auQuestions,
      qaEnabled: true,
    });
    setQsetOpen(false);
    setTab("qa");
  ...
  const drawNextQuestion = () => {
    if (auQaPool.length === 0) {
      toast("대기 중인 질문이 없습니다 — 질문 리스트를 추가해 주세요");
      return;
    }
    ...
    patchAuData({
      ...
      questions: [
        { no, q, date: new Date().toISOString().slice(0, 10), answers: [] },
        ...auQuestions,
      ],
      qaEnabled: true,
    });
  ...
  const saveQNote = () => {
    if (!qNote) return;
    patchAuData({
      questions: auQuestions.map((q) =>
        q.no === qNote.no ? { ...q, note: qNote.text.trim() || undefined } : q,
      ),
    });
    setQNote(null);
    toast("질문 설명을 저장했습니다");
  ...
  const canEditAns = (a: QaAnswer) =>
    a.authorId ? a.authorId === user?.id : isAdmin;
  const canDelAns = (a: QaAnswer) =>
    isAdmin || (!!a.authorId && a.authorId === user?.id);
  ...
  const saveAnsEdit = () => {
    ...
    const patch = {
      text: ansEdit.text.trim() || target.text,
      note: ansEdit.note.trim() || undefined,
    };
    if (target.rowId) {
      setQaRows(
        qaRows.map((r) => (r.id === target.rowId ? { ...r, ...patch } : r)),
      );
    } else {
      patchAuData({
        questions: auQuestions.map((q) =>
        ...
        patchAuData({
          questions: auQuestions.map((q) =>
            q.no === qNo
              ? {
                  ...q,
                  answers: q.answers.filter((_, i) => i !== target.legacyIdx),
                }
              : q,
          ),
        });
      ...
      updateRel({
        aus: rel.aus.map((a) =>
          a.id === au.id
            ? {
                ...a,
                mset: { ...a.mset, [cid]: { ...a.mset?.[cid], faceCrop: c } },
              }
            : a,
        ),
      });
  ...
  return (
    <section className="page page-rel-detail">
      ...
      {(() => {
        const hdrId = isBaseAu
          ? rel.headerImgId
          : (au?.headerImgId ?? undefined);
        ...
        <div className="au-list">
          ...
          {rel.aus.map((a, i) => {
            ...
            return (
              <div
                key={a.id}
                className={`au-item ${auId === a.id ? "on" : ""}`}
                onClick={() => {
                  setAuId(a.id);
                  setArtIdx(0);
                  setQaNo(null);
              ...
              >
                <CroppedBlobImg
                  fileRef={thumb}
                  crop={isBase ? rel.thumbCrop : undefined}
          ...
          {isAdmin && (
            <div
              className="au-item add"
              data-tip="AU 추가/관리"
              onClick={() => setAuOpen(true)}
            >
              ＋
            </div>
          )}
        ...
        {isAdmin && (
          <>
            ...
            <button
              ...
              onClick={() =>
                router.push(
                  `/rels/${rel.id}/edit${isBaseAu ? "" : `?au=${au!.id}`}`,
                )
              }
            >
        ...
        buttons={[
          {
            ...
            onClick: () => {
              ...
              setQaRows(
                qaRows.filter((r) => !(r.relId === rel.id && r.auId === gone)),
              );
      ...
      <div className="rel-hero">
        ...
        {auCpTag && (
          <div className="cp-top">
            <span
              className="pill"
              style={
                auSt.cpTagBg || auSt.cpTagFg
                  ? {
                      background: auSt.cpTagBg,
                      color: auSt.cpTagFg,
                      borderColor: auSt.cpTagBg,
                    }
                  : undefined
              }
            >
        ...
        <h1
          style={{
            fontFamily: familyOf(auFont),
            color: auSt.nameColor,
      ...
      {isDuo ? (
        <div className="rel-body" style={{ fontFamily: familyOf(auBodyFont) }}>
          {pairSlots[0] ? (
            <MiniProf
              member={pairSlots[0]}
              char={charOf(pairSlots[0].charId)}
              isAdmin={isAdmin}
              ...
              onFaceCrop={(ref) =>
                setFaceEdit({
                  charId: pairSlots[0]!.charId,
                  ref,
                  crop: pairSlots[0]!.faceCrop,
                })
              }
          ...
          <div
            className={`rel-center ${single ? "one-mode" : ""}`}
            style={{
          ...
          >
            ...
            <div
              ...
              onClick={() => {
                const n = auArts.length;
                if (n > 1) setArtIdx((i) => (i + 1) % n);
              }}
              onContextMenu={(e) => {
            ...
            >
              {auArts.length > 0 ? (
                <>
                  ...
                  <CroppedBlobImg
                    fileRef={auArts[Math.min(artIdx, auArts.length - 1)]}
                    crop={rel.artCrops?.[curArt]}
                    ph=""
                    label="MAIN ILLUST"
                  />
                  {auArts.length > 1 && (
                    ...
                    >
                      {auArts.map((_, i) => (
                        <i
                          ...
                          style={{
                            width:
                              i === Math.min(artIdx, auArts.length - 1)
                                ? 16
                                : 6,
                            height: 6,
                            borderRadius: 4,
            ...
            {hasFull && (
              ...
              >
                <button
                  className={!single ? "on" : ""}
                  onClick={() => setOneMode(false)}
                >
                  전신
                </button>
                <button
                  className={single ? "on" : ""}
                  onClick={() => setOneMode(true)}
                >
                  일러스트
                </button>
              </div>
          ...
          {pairSlots[1] ? (
            <MiniProf
              member={pairSlots[1]}
              char={charOf(pairSlots[1].charId)}
              isAdmin={isAdmin}
              ...
              onFaceCrop={(ref) =>
                setFaceEdit({
                  charId: pairSlots[1]!.charId,
                  ref,
                  crop: pairSlots[1]!.faceCrop,
                })
              }
        ...
        <div
          className="multi-body"
          style={{ fontFamily: familyOf(auBodyFont) }}
        >
          <div className="panel flush" style={{ padding: "6px 0" }}>
            {rel.members.map((m) => {
              ...
              return (
                <div
                  key={m.charId}
                  className="mrow"
                  style={{ ["--cc" as string]: rgbTriple(c.color) }}
                ...
                >
                    ...
                    {unreg ? (
                      ...
                      <>
                        <b
                          style={{
                            fontFamily: familyOf(
                              findChar(chars, m.charId)?.fontId,
                            ),
                          }}
                        >
                          {findChar(chars, m.charId)?.name}
                        </b>
                        <small>이 AU의 프로필 미등록 — 눌러서 등록</small>
                      </>
                    ) : (
                      <>
                        <b style={{ fontFamily: familyOf(c.fontId) }}>
                          {c.name}
                        </b>
                        <i>{c.sub}</i>
                        <small>
                  ...
                  <div className="gem-mini">
                    {(c.colors ?? []).slice(0, 3).map((p) => (
                      <i key={p.hex + p.label} style={{ background: p.hex }} />
                    ))}
                  </div>
                  {isAdmin && (
                    <span
            ...
            {isAdmin && rel.members.length < 6 && (
              <div className="mrow add" onClick={() => setMemberOpen(true)}>
                ＋ ADD MEMBER (최대 6인)
              </div>
            )}
          </div>
                  ...
                  >
                    {auArts.map((_, i) => (
                      <i
                        ...
                        style={{
                          width:
                            i === Math.min(artIdx, auArts.length - 1) ? 16 : 6,
                          height: 6,
                          borderRadius: 4,
                          background:
                            i === Math.min(artIdx, auArts.length - 1)
      ...
      {/* 타임라인 / 페어 문답 탭 (v1.8) */}
      <div
        className={`panel timeline ${!isDuo ? "multi" : ""}`}
        style={{ fontFamily: familyOf(auBodyFont) }}
      >
        <div className="rel-tabs">
          <button
            ...
            onClick={() => setTab("tl")}
          >
            <span className="lb-pc">TIMELINE</span>
            <span className="lb-m">T</span>
          </button>
          ...
          {qaOn && (
            <button
              ...
              onClick={() => setTab("qa")}
            >
              <span className="lb-pc">QUESTIONS</span>
              <span className="lb-m">Q</span>
            </button>
          )}
          {isAdmin && !qaOn && (
            <button
              data-tip="QUESTIONS 섹션 추가"
              style={{ color: "var(--faint)", fontSize: 14, padding: "0 6px" }}
              onClick={() => setQsetOpen(true)}
            >
              ＋
            </button>
          )}
          {isAdmin && (
            ...
            >
              ...
              {tab === "tl" ? (
                ...
                >
                  <span className="lb-pc">＋ ADD RECORD</span>
                  <span className="lb-m">＋</span>
                </button>
              ) : (
                <>
                  <button
                    ...
                    onClick={() => setQsetOpen(true)}
                  >
                    <span className="lb-pc">＋ 질문 리스트</span>
                    <span className="lb-m">≡</span>
                  </button>
                  ...
                  {curQa && (
                    ...
                    >
                      <span className="lb-pc">질문 건너뛰기</span>
                      <span className="lb-m">⏭</span>
                    </button>
                  )}
                  ...
                  {auQaPool.length > 0 && (
                    <button
                      className={curQa ? "btn btn-ghost" : "btn btn-dark"}
                      style={{ height: 35, padding: "0 14px", fontSize: 11.5 }}
                      data-tip={`대기 질문 ${auQaPool.length}개`}
                      onClick={drawNextQuestion}
                    >
                      <span className="lb-pc">
                        {curQa ? "완료 — 다음 질문" : "질문 받기"}
                      </span>
                      <span className="lb-m">↻</span>
                    </button>
                  )}
                  ...
                  >
                    <span className="lb-pc">＋ ADD QUESTION</span>
                    <span className="lb-m">＋</span>
                  </button>
                </>
              )}
        ...
        {tab === "tl" ? (
          tlSort ? (
            ...
            <DragList
              ...
              keyOf={(x) => x.key}
              onReorder={(list) =>
                patchAuData({ timeline: list.map((x) => x.item) })
              }
              render={({ item }) => (
                <div
                  style={{
            ...
            <div>
                ...
                >
                  ...
                  {item.says.map((s, j) => {
                    ...
                    return (
                      <div
                        key={j}
                        className={`tl-say ${sideOf(s.charId)}`}
                        style={{
                          ["--cc" as string]: rgbTriple(c?.color ?? "#5d636d"),
                        }}
                      >
                        <div
                          className="who"
                          style={{ fontFamily: familyOf(c?.fontId) }}
                        >
                          {c?.name}
                        </div>
              ...
              {auTimeline.length === 0 && (
                <p className="hint">
                  타임라인이 비어 있습니다 — 우상단 [＋ ADD RECORD]로 추가
                </p>
              )}
            </div>
          )
              ...
              {curQa ? (
                <>
                  ...
                  <div
                    ...
                    ref={ansRef}
                    onScroll={onAnsScroll}
                  >
                    <div className="qa-no">
                      TODAY&apos;S QUESTION · Q.
                      {String(curQa.no).padStart(3, "0")}
                      ...
                      {isAdmin && (
                        <small
                          ...
                          onClick={() =>
                            setQNote({ no: curQa.no, text: curQa.note ?? "" })
                          }
                        >
                          {curQa.note ? "설명 수정" : "＋ 설명"}
                        </small>
                    ...
                    <div className="qa-date" style={{ textAlign: "right" }}>
                      {curQa.date.replace(/-/g, ".")}
                    </div>
                    ...
                    {!qaHidden &&
                      curAnswers.map((a, i) => {
                        ...
                        return (
                          <div
                            key={i}
                            className={`qa-ans ${sideOf(a.charId) === "r" ? "r" : ""}`}
                            style={{
                              ["--cc" as string]: rgbTriple(
                                c?.color ?? "#5d636d",
                              ),
                            }}
                            /* 수정·부연·삭제는 우클릭 메뉴로 — 늘 떠 있으면 답변 줄이 난잡해진다 (사용자 확정) */
                            onContextMenu={(e) => {
                              if (!(canEditAns(a) || isAdmin || canDelAns(a)))
                                return;
                              e.preventDefault();
                  ...
                  {!qaHidden && answerableIds.length > 0 && (
                    <div className="qa-input">
                      ...
                      <div
                        ...
                        onClick={(e) => {
                          if (isDuo && answerableIds.length > 1) {
                            ...
                            setQaChar(
                              answerableIds[
                                (answerableIds.indexOf(cur) + 1) %
                                  answerableIds.length
                              ],
                            );
                      ...
                      >
                        <CharFace
                          c={charOf(qaChar ?? answerableIds[0])}
                          className="f"
                        />
                        <small
                          style={{
                            fontFamily: familyOf(
                              charOf(qaChar ?? answerableIds[0])?.fontId,
                            ),
                          }}
                        >
                          {charOf(qaChar ?? answerableIds[0])?.name}
                          {answerableIds.length > 1 ? " ▾" : ""}
                        </small>
                        {qaPickPos &&
                          createPortal(
                            <div
                ...
                <p className="hint">
                  등록된 문답이 없습니다 — 우상단 [＋ 질문 추가]로 시작해 보세요
                </p>
              )}
            </div>
            <div className="qa-list">
              ...
              <div className="qa-scroll">
                {qaFiltered.map((q) => (
                  ...
                  <div
                    key={q.no}
                    className={`qa-item ${curQa?.no === q.no ? "on" : ""}`}
                    onClick={() => setQaNo(q.no)}
                    data-tip={
                      isAdmin ? "우클릭 — 리스트로 되돌리기" : undefined
                    }
                    onContextMenu={(e) => {
                      if (!isAdmin) return;
                      e.preventDefault();
                      setQaCtx({ x: e.clientX, y: e.clientY, no: q.no });
                    }}
                  >
                    <b>
                      Q.{String(q.no).padStart(3, "0")} {q.q}
                    </b>
                    <small>
                      {q.date.slice(5).replace("-", ".")} · 답변{" "}
                      {answersOf(q.no).length}
                    </small>
                  </div>
                ))}
              </div>
        ...
        <div className="g2" style={{ marginTop: 16 }}>
          {!au?.hideRp && (
            <div
              ...
              style={{
                margin: 0,
                ...(au?.hideLog ? { gridColumn: "1/-1" } : null),
              }}
            >
              <h4>
                역극{" "}
                <span className="more" onClick={() => router.push("/rp")}>
                  더보기 ›
                </span>
              </h4>
              {relRooms.length > 0 ? (
          ...
          {!au?.hideLog && (
            <div
              ...
              style={{
                margin: 0,
                ...(au?.hideRp ? { gridColumn: "1/-1" } : null),
              }}
            >
              <h4>
                로그{" "}
                <span className="more" onClick={() => router.push("/trpg")}>
                  더보기 ›
                </span>
              </h4>
              {relLogs.length > 0 ? (
                ...
                <p className="hint" style={{ margin: 0 }}>
                  연동된 로그가 없습니다 — 로그 등록 시 자관을 선택하면 여기에
                  표시
                </p>
              )}
            </div>
      ...
      <Modal
        ...
        actions={
          <>
            ...
            <button className="btn btn-dark" onClick={addMember}>
              ADD
            </button>
          </>
        }
        ...
        <div style={{ display: "grid", gap: 10 }}>
          <div style={{ display: "flex", gap: 16 }}>
            <KRadio
              ...
              value="exist"
              current={mMode}
              onChange={(v) => setMMode(v as "exist")}
              label="기존 캐릭터"
            />
            <KRadio
              name="mm"
          ...
          {mMode === "exist" ? (
              ...
              >
                {candidates
                  ...
                  .map((c) => (
                    <div
                      key={c.id}
                      onClick={() => setMCharId(mCharId === c.id ? "" : c.id)}
                      style={{
                        display: "flex",
                        alignItems: "center",
                        gap: 8,
            ...
            <>
              ...
              <div>
                <label className="k-label" style={{ marginBottom: 7 }}>
                  회원 권한 (선택)
                </label>
          ...
          <KInput
            placeholder="캐릭터 한마디 (선택 — 타이틀 옆 인용구)"
            value={mQuote}
            onChange={(e) => setMQuote(e.target.value)}
          />
        </div>
      </Modal>
      ...
      <Modal
        open={tlOpen}
        onClose={closeTl}
        title={tlEditIdx == null ? "타임라인 항목 추가" : "타임라인 항목 수정"}
        desc="설명 / 한마디 중 하나는 필수 · 시기 라벨은 선택 · 한마디는 여러 개 추가 가능"
        dirty={!!(tEra || tDesc || tSays.some((x) => x.text))}
        actions={
          <>
            <button className="btn btn-ghost" onClick={closeTl}>
              CANCEL
            </button>
            <button className="btn btn-dark" onClick={addTlItem}>
              {tlEditIdx == null ? "ADD" : "SAVE"}
            </button>
          </>
        }
      >
        <div style={{ display: "grid", gap: 10 }}>
          <KInput
            placeholder="시기 라벨 (선택)"
            value={tEra}
            onChange={(e) => setTEra(e.target.value)}
          />
          <KTextarea
            placeholder="설명"
            value={tDesc}
          ...
          <label className="k-label" style={{ margin: 0 }}>
            한마디 — 캐릭터를 바꿔가며 주고받는 대화로 쌓을 수 있습니다
          </label>
          {tSays.map((s) => (
            <div
              key={s.id}
            ...
            >
              <KSelect
                ...
                onChange={(v) =>
                  setTSays((l) =>
                    l.map((x) => (x.id === s.id ? { ...x, charId: v } : x)),
                  )
                }
              ...
              <span
                className="fx"
                onClick={() => setTSays((l) => l.filter((x) => x.id !== s.id))}
              >
                ✕
              </span>
            </div>
      ...
      <Modal
        ...
        actions={
          <button className="btn btn-dark" onClick={() => setAuOpen(false)}>
            닫기
          </button>
        }
      >
          ...
          {rel.aus.map((a) => (
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                />
                <KInput
                  value={a.id === "base" ? rel.catchphrase : a.catchphrase}
                  placeholder="캐치프레이즈"
                  onChange={(e) => {
                ...
                {a.id !== "base" && (
                  <>
                    ...
                    <span
                      ...
                      style={{
                        flexShrink: 0,
                        opacity: rel.aus.indexOf(a) <= 1 ? 0.3 : 1,
                      }}
                      data-tip="위로"
                      onClick={() => {
                    ...
                    <span
                      ...
                      style={{
                        flexShrink: 0,
                        opacity:
                          rel.aus.indexOf(a) >= rel.aus.length - 1 ? 0.3 : 1,
                      }}
                      data-tip="아래로"
                      onClick={() => {
                        const i = rel.aus.findIndex((x) => x.id === a.id);
                    ...
                    <span
                      ...
                      onClick={() =>
                        del.ask(
                          `AU 「${a.label}」를 삭제하시겠습니까?`,
                          () => {
                            updateRel({
                              aus: rel.aus.filter((x) => x.id !== a.id),
                            });
                            if (auId === a.id) setAuId("base");
                          },
                          "이 AU의 일러·타임라인·문답이 함께 삭제되며 복구할 수 없습니다.",
                        )
                      }
                    >
                      ✕
              ...
              <div style={{ display: "flex", gap: 16 }}>
                <KCheck
                  label={
                    <span style={{ fontSize: 11.5 }}>역극 리스트 숨김</span>
                  }
                  checked={!!a.hideRp}
                  onChange={(v) =>
                    updateRel({
                      aus: rel.aus.map((x) =>
                        x.id === a.id ? { ...x, hideRp: v || undefined } : x,
                      ),
                    })
                  }
                />
                <KCheck
                  label={
                    <span style={{ fontSize: 11.5 }}>로그 리스트 숨김</span>
                  }
                  checked={!!a.hideLog}
                  onChange={(v) =>
                    updateRel({
                      aus: rel.aus.map((x) =>
                        x.id === a.id ? { ...x, hideLog: v || undefined } : x,
                      ),
                    })
                  }
                />
          ...
          <div style={{ display: "grid", gap: 8 }}>
            <div style={{ display: "flex", gap: 8 }}>
              <KInput
                placeholder="AU 이름"
                value={auLabel}
                onChange={(e) => setAuLabel(e.target.value)}
                style={{ maxWidth: 130 }}
              />
              <KInput
                placeholder="캐치프레이즈"
                value={auCatch}
                onChange={(e) => setAuCatch(e.target.value)}
              />
            </div>
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              ...
              <button
                ...
                onClick={() => {
                  if (!auLabel.trim()) {
                    toast("AU 이름을 입력해 주세요");
                    return;
                  }
                  const na: RelAu = {
                    id: newId(),
                    label: auLabel.trim(),
                    catchphrase: auCatch.trim(),
                    cp: auCp,
                    timeline: [],
                    questions: [],
                  };
      ...
      {artCtx &&
        createPortal(
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <div className="ctx-ttl">
              중앙 일러
              {auArts.length > 1
                ? ` ${Math.min(artIdx, auArts.length - 1) + 1}/${auArts.length}`
                : ""}
            </div>
            <button
              onClick={() => {
            ...
            {rel.artCrops?.[artCtx.ref] && (
              <button
                onClick={() => {
                  saveArtCrop(artCtx.ref, undefined);
                  setArtCtx(null);
                }}
              >
                위치 지정 해제
      ...
      {artCropOpen && (
        <RelArtCropModal
          ...
          crop={rel.artCrops?.[artCropOpen.ref]}
          onClose={() => setArtCropOpen(null)}
          onApply={(c) => {
            saveArtCrop(artCropOpen.ref, c);
            setArtCropOpen(null);
          }}
        />
      )}
      ...
      <Modal
        open={qsetOpen}
        onClose={() => setQsetOpen(false)}
        small
        title="질문 리스트 추가"
        desc="환경설정 > 자관 질문의 세트에서 골라 현재 AU의 QUESTIONS에 넣습니다"
        actions={
          <button className="btn btn-ghost" onClick={() => setQsetOpen(false)}>
            CANCEL
          </button>
        }
      >
        <div style={{ display: "grid", gap: 8 }}>
          {[...qsets]
            .sort(
              (a, b) =>
                (a.cat === (auCpTag ?? "cp") ? -1 : 0) -
                (b.cat === (auCpTag ?? "cp") ? -1 : 0),
            )
            .map((s) => (
              <button
                key={s.id}
                type="button"
                onClick={() => addQuestionSet(s)}
                style={{
                  display: "flex",
                  alignItems: "center",
                  gap: 10,
      ...
      <Modal
        ...
        actions={
          <>
            <button className="btn btn-ghost" onClick={() => setQOpen(false)}>
              CANCEL
            </button>
            <button className="btn btn-dark" onClick={addQuestion}>
              ADD
            </button>
          </>
        }
      ...
      <Modal
        ...
        actions={
          <>
            <button className="btn btn-ghost" onClick={() => setAnsEdit(null)}>
              CANCEL
            </button>
            <button className="btn btn-dark" onClick={saveAnsEdit}>
              SAVE
            </button>
          </>
        }
      >
        {ansEdit && (
          <div style={{ display: "grid", gap: 9 }}>
            {canEditAns(
              curAnswers[ansEdit.idx] ?? { charId: "", text: "" },
            ) && (
              <KTextarea
                value={ansEdit.text}
                onChange={(e) =>
                  setAnsEdit((s) => s && { ...s, text: e.target.value })
                }
                style={{ minHeight: 60 }}
              />
            )}
            ...
            {(isAdmin ||
              ...
              <div>
                <label className="k-label" style={{ marginBottom: 5 }}>
                  부연설명 — 말풍선에 마우스를 올리면 표시 (비우면 없음)
                </label>
                <KTextarea
                  value={ansEdit.note}
                  onChange={(e) =>
                    setAnsEdit((s) => s && { ...s, note: e.target.value })
                  }
                  style={{ minHeight: 46 }}
                />
              </div>
      ...
      <Modal
        ...
        actions={
          <>
            <button className="btn btn-ghost" onClick={() => setQNote(null)}>
              CANCEL
            </button>
            <button className="btn btn-dark" onClick={saveQNote}>
              SAVE
            </button>
          </>
        }
        ...
        {qNote && (
          <div>
            ...
            <KTextarea
              value={qNote.text}
              onChange={(e) =>
                setQNote((s) => s && { ...s, text: e.target.value })
              }
              style={{ minHeight: 70 }}
            />
          </div>
      ...
      {/* 타임라인 우클릭 메뉴 (v2.0) */}
      {tlCtx &&
        auTimeline[tlCtx.idx] &&
        createPortal(
          <div
            className="ctx-menu on"
            style={{ left: tlCtx.x, top: tlCtx.y }}
            onClick={(e) => e.stopPropagation()}
          >
            <div className="ctx-ttl">
              {auTimeline[tlCtx.idx].era || "타임라인 항목"}
            </div>
            <button
              onClick={() => {
                const i = tlCtx.idx;
            ...
            <button
              ...
              onClick={() => {
                ...
                del.ask("타임라인 항목을 삭제하시겠습니까?", () =>
                  patchAuData({
                    timeline: auTimeline.filter((_, j) => j !== i),
                  }),
                );
              }}
            >
      ...
      {qaCtx &&
        createPortal(
          <div
            className="ctx-menu on"
            style={{ left: qaCtx.x, top: qaCtx.y }}
            onClick={(e) => e.stopPropagation()}
          >
        ...
        createPortal(
          (() => {
            ...
            return (
              <div
                className="ctx-menu on"
                style={{ left: ansCtx.x, top: ansCtx.y }}
                onClick={(e) => e.stopPropagation()}
              >
                <div className="ctx-ttl">
                  {charOf(a.charId)?.name ?? "답변"}
                </div>
