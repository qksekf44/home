...
export default function BoardDetailPage() {
  ...
  const blocked = useHrefBlock(
    post && (bid === MAIN_BOARD_ID ? "/board" : extraBoardHref(bid)),
  );
  // loaded 이후에만 본문 렌더 (SSR/하이드레이션 불일치 방지)
  const html = useMemo(
    () => (post && loaded ? renderBody(post.mode, post.body) : ""),
    [post, loaded],
  );
  ...
  const addComment = () => {
    ...
    if (replyTo) {
      ...
      for (const t of comments.filter(
        (x) => x.id === replyTo || x.parentId === replyTo,
      )) {
        const to = t.authorId;
  ...
  const removeComment = (c: Comment) => {
    const gone = (x: { id: string; parentId?: string }) =>
      x.id === c.id || x.parentId === c.id;
    if (cmtRows.some(gone)) setCmtRows(cmtRows.filter((x) => !gone(x)));
    if (post.comments.some(gone))
      update({ comments: post.comments.filter((x) => !gone(x)) });
  };
  ...
  const CmtRow = ({ c, depth }: { c: Comment; depth: number }) => (
    <div className={`cmt ${depth > 0 ? "reply-depth" : ""}`}>
      ...
      {(isAdmin || (user && c.authorId === user.id)) && (
        <small
          style={{ cursor: "var(--cur-pointer,pointer)", marginLeft: 8 }}
        ...
        <p style={{ fontSize: 11, color: "var(--faint)", marginBottom: 18 }}>
          ...
          {(post.tags ?? []).map((t) => (
            <span
              ...
              style={{
                marginLeft: 7,
                color: "color-mix(in srgb,var(--accent) 65%,var(--faint))",
              }}
            >
              #{t}
            </span>
      ...
      <div className="panel" style={{ padding: 0, marginTop: 16 }}>
        <div style={{ padding: "16px 18px" }}>
          ...
          >
            COMMENTS{" "}
            {comments.length > 0 && (
              <span style={{ color: "var(--accent)" }}>{comments.length}</span>
            )}
          </h4>
          {roots.map((c) => (
        ...
        {canComment ? (
          ...
          <div className={`cmt-input ${guestMode ? "guest" : ""}`}>
            ...
            <div
              className="ci-row"
              style={guestMode ? undefined : { display: "contents" }}
            >
              <KInput
        ...
        buttons={[
          {
            ...
            onClick: () => {
              ...
              setCmtRows(
                cmtRows.filter(
                  (c) => !(c.target === "post" && c.targetId === post.id),
                ),
              );
