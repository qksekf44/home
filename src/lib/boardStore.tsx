...
export const DEFAULT_BOARD_CATS: BoardBadge[] = [
  "잡담",
  "설정",
  "합작",
  "기타",
...
export const DEFAULT_GALLERY_CATS: BoardBadge[] = [
  "합작",
  "낙서",
  "커미션",
  "설정화",
...
const DEFAULTS: BoardSettings = {
  system: DEFAULT_BOARD_SYSTEM,
  cats: DEFAULT_BOARD_CATS,
  gallery: DEFAULT_GALLERY_BADGES,
  galleryCats: DEFAULT_GALLERY_CATS,
};
...
export const galleryCatsOf = (s: BoardSettings, secId: string): BoardBadge[] =>
  secId === MAIN_SEC
    ? s.galleryCats
    : (s.secGalleryCats?.[secId] ?? s.galleryCats);

/** 그 갤러리의 말머리를 담은 patch — 기본 갤러리면 예전 자리에 그대로 저장한다 */
const galleryCatsPatch = (
  s: BoardSettings,
  secId: string,
  cats: BoardBadge[],
  ...
  secId === MAIN_SEC
    ? { galleryCats: cats }
    : { secGalleryCats: { ...s.secGalleryCats, [secId]: cats } };
...
export function useBoardSettings() {
    ...
    try {
      ...
      if (raw) {
        ...
        setSt({
          ...
          ...DEFAULTS,
          ...p,
          system: DEFAULT_BOARD_SYSTEM.map(
            (d) => p.system?.find((s) => s.id === d.id) ?? d,
          ),
          cats: p.cats ?? DEFAULT_BOARD_CATS,
          gallery: DEFAULT_GALLERY_BADGES.map(
            (d) => p.gallery?.find((g) => g.id === d.id) ?? d,
          ),
  ...
  const patchSystem = useCallback(
    (id: string, p: Partial<BoardBadge>) =>
      apply((s) => ({
        ...s,
        system: s.system.map((b) => (b.id === id ? { ...b, ...p } : b)),
      })),
    [apply],
  );
  const patchCat = useCallback(
    (id: string, p: Partial<BoardBadge>) =>
      apply((s) => ({
        ...s,
        cats: s.cats.map((b) => (b.id === id ? { ...b, ...p } : b)),
      })),
    [apply],
  );
  ...
  const removeCat = useCallback(
    (id: string) =>
      apply((s) => ({ ...s, cats: s.cats.filter((b) => b.id !== id) })),
    [apply],
  );
  const setCats = useCallback(
    (cats: BoardBadge[]) => apply((s) => ({ ...s, cats })),
    [apply],
  );
  const patchGallery = useCallback(
    (id: string, p: Partial<BoardBadge>) =>
      apply((s) => ({
        ...s,
        gallery: s.gallery.map((b) => (b.id === id ? { ...b, ...p } : b)),
      })),
    [apply],
  );
  ...
  const mutGalleryCats = useCallback(
    (secId: string, fn: (cats: BoardBadge[]) => BoardBadge[]) =>
      apply((s) => ({
        ...s,
        ...galleryCatsPatch(s, secId, fn(galleryCatsOf(s, secId))),
      })),
    [apply],
  );
  const patchGalleryCat = useCallback(
    (secId: string, id: string, p: Partial<BoardBadge>) =>
      mutGalleryCats(secId, (cs) =>
        cs.map((b) => (b.id === id ? { ...b, ...p } : b)),
      ),
    [mutGalleryCats],
  );
  ...
  const removeGalleryCat = useCallback(
    (secId: string, id: string) =>
      mutGalleryCats(secId, (cs) => cs.filter((b) => b.id !== id)),
    [mutGalleryCats],
  );
  const setGalleryCats = useCallback(
    (secId: string, cats: BoardBadge[]) => mutGalleryCats(secId, () => cats),
    [mutGalleryCats],
  );
  return {
    st,
...
export function badgeFor(
  st: BoardSettings,
  p: { notice?: boolean; secret?: boolean; category: string },
  cats?: BoardBadge[],
): BoardBadge {
  if (p.notice) return st.system[0];
  if (p.secret) return st.system[1];
  return (
    (cats ?? st.cats).find((c) => c.label === p.category) ?? {
      id: "etc",
      label: p.category,
      bg: "#eef0f2",
      border: "#d7dae0",
...
export const boardHref = (id: string) =>
  id === MAIN_BOARD_ID ? "/board" : `/board?b=${id}`;
...
export function boardBadgeStyle(b?: BoardBadge): React.CSSProperties {
  return {
    ...
    borderRadius: 999,
    lineHeight: "calc(11px*var(--fs,1))",
    background: b?.bg ?? "#eef0f2",
    border: `1px solid ${b?.border ?? "#d7dae0"}`,
    color: b?.fg ?? "#5d636d",
    fontSize: "calc(10.5px*var(--fs,1))",
    fontWeight: 700,
    letterSpacing: ".05em",
    fontFamily: "var(--sans)",
    whiteSpace: "nowrap",
