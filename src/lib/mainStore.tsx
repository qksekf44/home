...
export const WIDGET_META: Record<WidgetType, { title: string; desc: string }> =
  {
    ...
    memoboard: {
      title: "STICKY",
      desc: "스티커 메모 미니보드 — 클릭 시 메모장 (4.6)",
    },
    apply: {
      title: "COMMISSION",
      desc: "커미션 신청자 — 마감 빠른 순 (몇 명까지 볼지 설정)",
    },
  };
...
export function widgetLabel(widgets: WidgetConf[], w: WidgetConf): string {
  ...
  return same.length > 1
    ? `${t} ${same.findIndex((x) => x.id === w.id) + 1}`
    : t;
}
...
const EDIT_PAGE_NAMES =
  "메인 · 신청자 리스트 · 캐릭터 · 자관 · 커미션 · 갤러리 · 도토리 · TRPG 캐릭터 · 플레이기록 · TRPG 로그";
...
export function MainStoreProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        const merged = [
          ...kept,
          ...DEFAULT_STATE.widgets.filter(
            (w) => !ids.has(w.id) && !removed.has(w.id),
          ),
        ];
        setState({
          ...DEFAULT_STATE,
  ...
  const updateWidget = useCallback(
    (id: string, patch: Partial<WidgetConf>, opts?: { persist?: boolean }) => {
      setState((s) => {
        const n = {
          ...s,
  ...
  const addWidget = useCallback((type: WidgetType, col: 1 | 2 | 3): string => {
    ...
    setState((s) => {
      ...
      if (!MULTI_TYPES.includes(type) && s.widgets.some((w) => w.type === type))
        return s;
      ...
      const maxY = Math.max(
        60,
        ...s.widgets
          .filter((w) => w.enabled && w.col === col && w.ay != null)
  ...
  const setMobileOff = useCallback(
    (id: string, v: boolean) => {
      setState((s) => {
        const n = {
          ...s,
          widgets: s.widgets.map((w) => (w.id === id ? { ...w, mOff: v } : w)),
        };
        persist(n);
        return n;
      });
  ...
  const setMobileOrder = useCallback(
    (ids: string[]) => {
      setState((s) => {
        const n = { ...s, mobileOrder: ids };
        persist(n);
        return n;
      });
    },
  ...
  return (
    <Ctx.Provider
      value={{
        state,
        editOn,
        editAvailable: EDIT_PAGES.includes(pathname),
        gridOn,
        setGridOn,
        toggleEdit,
        requestExit,
    ...
    >
      ...
      <ConfirmModal
        ...
        buttons={[
          { label: "확인", kind: "dark", onClick: () => setNotice(null) },
        ]}
      />
    </Ctx.Provider>
  );
